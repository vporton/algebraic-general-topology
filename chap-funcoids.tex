...
Thus
\[
\Compl f=\bigsqcap^{\mathsf{ComplFCD}(A;B)}\up^{\mathsf{ComplFCD}(A;B)}f=\Cor^{\mathsf{ComplFCD}(A;B)}f.
\]
\end{proof}
\begin{thm}
...
for every $p\in\Ob f$ and $C\in\subsets\Ob f$. \fxerror{This definition is wrong (not the same as in the customary general topology).
Instead it should be $\supfun f\supfun{f^{-1}}C\asymp\rsupfun f\{p\}\Leftarrow\uparrow^{\Ob f}\{p\}\asymp\supfun{f^{-1}}C$.}
\end{defn}
\begin{prop}
Endofuncoid $f$ is regular iff: \fxerror{Every below item must be quantified with $\forall C\forall p$}
\begin{enumerate}
...
\item $\uparrow^{\Ob f}\{p\}\sqsubseteq\rsupfun{f^{-1}\circ f\circ f^{-1}}C \Rightarrow p\in C$;
\item $\rsupfun{f^{-1}\circ f\circ f^{-1}}C\sqsubseteq\uparrow^{\Ob f}C$;
  \fxerror{Counter-example: $f=\Delta\times^{\mathsf{FCD}}\Delta$ and $C=\{0\}$ ($\Delta$ is defined in the section ``Counterexamples''.)}
