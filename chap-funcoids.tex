...
\begin{thm}
\label{fcd-as-cont}Fix sets $A$ and $B$. Let $L_{F}=\mylamdba 
...
{C}\sqsupseteq\supfun{\xi\circ\nu}\mathcal{A}\Leftrightarrow\mathcal{A}\suprel{
(\xi\circ\nu)^{\circledast}}\mathcal{C}.
\end{multline*}
...
\begin{thm}
\label{pf-fcd-atom-args}For every funcoid $f$ and 
...
\begin{equation}
\forall X\in a,Y\in b\exists x\in\atoms\uparrow X,y\in\atoms\uparrow 
X:x\mathrel\delta y\Rightarrow a\mathrel\delta b\label{at-rel-cond}
\end{equation}
can be continued to the relation $\suprel f$ for a unique 
$f\in\mathsf{FCD}(A;B)$;
\begin{equation}
\mathcal{X}\suprel f\mathcal{Y}\Leftrightarrow\exists 
x\in\atoms\mathcal{X},y\in\atoms\mathcal{X}:x\mathrel\delta y\label{at-rel-eq}
\end{equation}
for every $\mathcal{X}\in\mathscr{F}(A)$, $\mathcal{Y}\in\mathscr{F}(B)$.
...
\[
\forall X\in a,Y\in b\exists x\in\atoms\uparrow X,y\in\atoms\uparrow 
X:x\mathrel\delta y\Leftrightarrow a\mathrel\delta b.
\]
...
\begin{align*}
\forall X\in a,Y\in b\exists x\in\atoms\uparrow X,y\in\atoms\uparrow 
X:x\mathrel\delta y & \Leftrightarrow\\
...
\forall f\in R,X\in a,Y\in b\exists x\in\atoms\uparrow X,y\in\atoms\uparrow 
...
So the lattice $\mathsf{FCD}(A;B)$ is separable by the theorem 
...
\exists a\in\mathscr{F}(\Src f),b\in\mathscr{F}(\Dst 
...
\item \label{cfcd:r-set}$\forall\subsets\mathscr{T}(\Src f),J\in\mathscr{T}(\Dst 
...
$X\times^{\mathsf{FCD}}Y\sqcup\overline{X}\times^{\mathsf{FCD}}\top^{\mathscr{F}
(B)}$.\end{lem}
\begin{proof}
...
\[
\bigsqcap_{Y\in\up\rsupfun fX}\rsupfun{g(X;Y)}X=\bigsqcap_{Y\in\up\rsupfun 
fX}\uparrow^{B}Y=\rsupfun fX;
\]
consequently
\[
...
Let $\beta\in\Src f$.
\[
\rsupfun f\uparrow^{A}\{\beta\}=\bigsqcup_{\alpha\in 
A}\rsupfun{\uparrow^{A}\{\alpha\}\times^{\mathsf{FCD}}F(\alpha)}\uparrow^{A}\{
\beta\}=F(\beta).
\]
...
Then
\begin{multline*}
\rsupfun f\uparrow^{A}\{\alpha'\}=\text{(because 
\ensuremath{\uparrow^{A}\{\alpha'\}} is principal)}=\\
\bigsqcup_{\alpha\in 
A}\supfun{\uparrow^{A}\{\alpha\}\times^{\mathsf{FCD}}G(\alpha)}\uparrow^{A}\{
\alpha'\}=\supfun{\uparrow^{A}\{\alpha'\}\times^{\mathsf{FCD}}G(\alpha')}
\uparrow^{A}\{\alpha'\}=G(\alpha').
\end{multline*}
\end{proof}
\begin{cor}
...
$G(\alpha)=\rsupfun{f^{-1}}\uparrow^{A}\{\alpha\}$
...
the formula $\rsupfun{\Compl f}\uparrow^{\Src f}\{\alpha\}=\rsupfun 
f\uparrow^{\Src f}\{\alpha\}$
for $\alpha\in\Src f$.
\end{defn}
...
$\rsupfun R\uparrow^{\Src f}\{\beta\}=\bigsqcup_{\alpha\in\Src 
f}\rsupfun{f|_{\uparrow^{\Src f}\{\alpha\}}}\uparrow^{\Src f}\{\beta\}=\rsupfun 
f\uparrow^{\Src f}\{\beta\}$
...
~
\[
\CoCompl((\Compl g)\circ f)=\Compl(g\circ(\CoCompl f))=(\Compl g)\circ(\CoCompl 
f).
\]
\end{cor}
\begin{proof}
...
After variable replacement $\CoCompl((\Compl g)\circ f)=(\Compl g)\circ(\CoCompl 
f)$.\end{proof}
\begin{prop}
For every composable funcoids $f$ and $g$
\begin{enumerate}
...
\item [{\ref{compl2-eq}}] ~
\begin{multline*}
\rsupfun{g\circ(\Compl f)}\uparrow^{\Src f}\{x\}=\supfun g\rsupfun{\Compl 
f}\uparrow^{\Src f}\{x\}=\\
\supfun g\rsupfun f\uparrow^{\Src f}\{x\}=\rsupfun{g\circ f}\uparrow^{\Src 
f}\{x\}.
\end{multline*}
...
\[
\supfun{\left(\bigsqcap G\right)\circ f}x=\supfun{\left(\bigsqcap 
G\right)}\supfun fx=\bigsqcap_{g\in G}\supfun g\supfun fx=\bigsqcap_{g\in 
...
\[
\alpha\ne\beta\Rightarrow\lnot(\uparrow^{\Ob f}\{\alpha\}\rsuprel f\uparrow^{\Ob 
f}\{\beta\}).
\]
\end{defn}
\begin{prop}
...
~
\begin{multline*}
\forall x,y\in\Ob f:(\uparrow^{\Ob f}\{x\}\rsuprel f\uparrow^{\Ob 
f}\{y\}\Rightarrow x=y)\Leftrightarrow\\
\forall x,y\in\Ob f:(\{x\}\uparrow^{\Ob f}\rsuprel{\Cor f}\uparrow^{\Ob 
f}\{y\}\Rightarrow x=y)\Leftrightarrow\Cor f\sqsubseteq1_{\Ob f}^{\mathsf{FCD}}.
\end{multline*}
\end{proof}
\begin{defn}
...
\uparrow^{\Src f} \{ p \} \Leftarrow \uparrow^{\Src f} \{ p \}
...
\uparrow^{\Src f} \{ p \} \Rightarrow \uparrow^{\Src f} \{ p \}
\nasymp \langle f^{- 1} \rangle C$;

(by definition of funcoids)

$C \nasymp \supfun{f} \langle f^{- 1} \rangle \supfun{f}
\uparrow^{\Src f} \{ p \} \Rightarrow C \nasymp \supfun{f}
\uparrow^{\Src f} \{ p \}$;

$\supfun{f} \langle f^{- 1} \rangle \supfun{f}
\uparrow^{\Src f} \{ p \} \sqsubseteq \supfun{f}
\uparrow^{\Src f} \{ p \}$;

$\langle f \circ f^{- 1} \circ f \rangle \uparrow^{\Src f} \{ p \}
\sqsubseteq \supfun{f} \uparrow^{\Src f} \{ p \}$;
