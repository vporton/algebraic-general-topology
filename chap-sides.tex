...
\[ f\sqsubseteq g \Leftrightarrow \supfun{f}\sqsubseteq\supfun{g} \Leftrightarrow \supfun{f^{-1}}\sqsubseteq\supfun{g^{-1}}. \]
...
\begin{prop}
If $f$ and~$g$ are Galois connections (between some fixed posets \fxerror{join-semilattices}), then there exists a Galois connection~$f\sqcup g$
determined by the formula~$\supfun{f\sqcup g}p = \supfun{f}p\sqcup\supfun{g}p$.
\end{prop}
