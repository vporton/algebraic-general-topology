...
\begin{enumerate}
  ...
  \item $\supfun{a}\bot^{\Src\supfun{a}} = \bot^{\Dst\supfun{a}}$.
...
\begin{defn}
\emph{System of sides} is system of presides from an ordered category with bounded Hom-sets with
a morphism $\id_X\in\Src\Upsilon$ for every object $\mathfrak{A}$ of~$\Src\Upsilon$ and $X\in\mathfrak{A}$
and the following additional axioms: \fxwarning{$\bot$ and $\top$ are defined only for specific Hom-sets.}
\begin{enumerate}
