...
\fxwarning{\emph{Objects of sides} should be \emph{objects of source of a system}.}
\end{defn}
...
\fxwarning{Rewrite below using the above definition.}
...
\item $f\circ(X\times Y) = X\times\supfun{f}Y$
\end{enumerate}
\end{defn}
...
\fxwarning{Not only for faithful, if we define $A\times B=\id_B\circ\top\circ\id_A$.}
\end{cor}
