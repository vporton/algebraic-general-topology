...
\begin{cor}
$\supfun{\bot}a=\bot$ for Galois connections from
a poset~$\mathfrak{A}$ with greatest element to a poset~$\mathfrak{B}$ with least element.
\fxwarning{Clarify.}
\end{cor}
...
\begin{prop}
  ...
  \fxwarning{Does we need also faithfulness? (or only special case for the side~$\bot$?)
  Oh well, $\supfun{f}x\sqsupset\bot \Rightarrow f\ne\bot \Rightarrow f\sqsupset\bot$.}
  \fxwarning{Check again.}
\end{prop}
