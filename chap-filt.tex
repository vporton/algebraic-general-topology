...
\begin{conjecture}
$\mathfrak{F}(Z(D\mathcal{A}))$ is order-isomorphic to~$D\mathcal{A}$ for every filter~$\mathcal{A}$ on a set.
\end{conjecture}

If the above conjecture will be found true, it is to be generalized for a wider class of filters than filters on a set.

\subsection{Informal problems}
