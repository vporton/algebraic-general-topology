...
(where $\mathfrak{U}$ is some fixed set) with the order $A\sqsubseteq B\Leftrightarrow A\supseteq B$
...
with binarily\emph{ join-closed core} such a filtrator $(\mathfrak{A};\mathfrak{Z})$
...
with binarily\emph{ meet-closed core} such a filtrator $(\mathfrak{A};\mathfrak{Z})$
...
Recall that \emph{filters} are nonempty sets $F$ without with $A,B\in F\Leftrightarrow\exists Z\in F:(Z\sqsubseteq A\wedge Z\sqsubseteq B)$
...
to prove that $F$ is an upper set. let $A\in F$ and $A\sqsubseteq B\in F$.
...
\end{proof}
\begin{lem}
If $S\in\subsets\mathfrak{Z}$ and for every $T\in\subsets\mathfrak{A}$
which is not the complement of empty set 
\[
...
\begin{proof}
Take $S=\{A,B\}$. Then $\forall Z\in\mathfrak{A}:(Z\sqsupseteq A\wedge Z\sqsupseteq B\Rightarrow Z\in S)\Leftrightarrow A\in S\vee B\in S$.
...
\begin{prop}
Every complete join infinite distributive lattice is starrish.\end{prop}
\begin{proof}
...
subset and $S\in\subsets\mathfrak{A}$, then $\bigsqcup^{\mathfrak{A}}S$
...
every $S\in\subsets\mathfrak{F}(\mathfrak{Z})$.
...
$S\in\subsets\mathfrak{I}(\mathfrak{Z})$.
...
and~$\sqcup$.
\begin{cor}
If $\mathfrak{Z}$ is a join-semilattice then $\mathfrak{A}$ is a
...
\[
R\supseteq\bigsqcap^{\mathfrak{F}(\mathfrak{Z})}S=\setcond{K_{0}\sqcap^{\mathfrak{Z}}\dots\sqcap^{\mathfrak{Z}}K_{n}}{K_{i}\in\mathcal{A}\text{ where }i=0,\dots,n\text{ for }n\in\mathbb{N}}=\mathcal{A}.
\]
...
\[
\bigsqcap^{\mathfrak{F}(\mathfrak{Z})}S=\setcond{K_{0}\sqcap^{\mathfrak{Z}}\dots\sqcap^{\mathfrak{Z}}K_{n}}{K_{i}\in\bigcup S\text{ where }i=0,\dots,n\text{ for }n\in\mathbb{N}}
\]
...
for $S\in\subsets\mathfrak{F}(\mathfrak{Z})\setminus\{\emptyset\}$
if $\mathfrak{Z}$ is a meet-semilattice;
\item $\mathcal{F}_{0}\sqcap^{\mathfrak{I}(\mathfrak{Z})}\dots\sqcap^{\mathfrak{I}(\mathfrak{Z})}\mathcal{F}_{m}=\bigcup\setcond{\uparrow(K_{0}\sqcup^{\mathfrak{Z}}\dots\sqcup^{\mathfrak{Z}}K_{m})}{K_{i}\in\mathcal{F}_{i}\text{ where }i=0,\dots,m}$
for $S\in\subsets\mathfrak{I}(\mathfrak{Z})\setminus\{\emptyset\}$
if $\mathfrak{Z}$ is a join-semilattice.
\end{enumerate}
\end{thm}
...
It follows from the fact that 
\[
\mathcal{F}_{0}\sqcap^{\mathfrak{F}(\mathfrak{Z})}\dots\sqcap^{\mathfrak{F}(\mathfrak{Z})}\mathcal{F}_{m}=\bigcup\setcond{K_{0}\sqcap^{\mathfrak{Z}}\dots\sqcap^{\mathfrak{Z}}K_{m}}{K_{i}\in\mathcal{F}_{i}\text{ where }i=0,\dots,m}
\]
...
\mathcal{A}\cap\bigsqcap^{\mathfrak{A}}S & =\\
...
\[
\forall K\in\mathcal{A}:\mathcal{B}\sqsubseteq\uparrow K
\]
...
\section{Characterization of Finitely Meet-Closed Filtrators}
\begin{thm}
...
our filtrator base is join-closed.
...
Now $\Cor'a\in\down a$ is obvious. Thus $\Cor'a\in\max\down a$.
\end{description}
\end{proof}
\begin{prop}
...
\item \label{cor-eq-mlat}$a$ is a filter on a complete lattice with greatest
element.
...
be a filtrator. \emph{Core star} of an element $a$ of a filtrator
is
\[
...
lattice filtrator with least element and finitely join-closed core
...
by the theorem~\ref{semifilt-joinclosed}, finitely meet-closed by
proposition~\ref{f-meet-closed}.
...
\begin{align*}
0\sqsubset x\sqsubset a & \Rightarrow\\
...
filtrator over a bounded distributive lattice with greatest element.
...
The filtrator $(\mathfrak{A};\mathfrak{Z})$ is finitely join-closed
...
Obviously $S'$ is nonempty and finitely meet-closed. So $S'$ is
...
$(\mathfrak{A};\mathfrak{Z})$ is finitely meet-closed (by the theorem
\ref{up-filt-crit}).
\end{description}
\end{proof}
\begin{thm}
...
If $\bot\in\bigsqcap^{\mathfrak{F}}S$ then there are $K_{i}\in\bigcup S$
such that $\bot\in\uparrow(K_{0}\sqcap^{\mathfrak{Z}}\ldots\sqcap^{\mathfrak{Z}}K_{n})$
that is $K_{0}\sqcap^{\mathfrak{Z}}\ldots\sqcap^{\mathfrak{Z}}K_{n}=\bot$
from which easily follows $\mathcal{F}_{0}\sqcap^{\mathfrak{F}}\ldots\sqcap^{\mathfrak{F}}\mathcal{F}_{n}=\bot$
...
(used the theorem \ref{atomistic-enough}) $\mathfrak{A}$ is atomic
...
with join-closed, finitely meet-closed and separable core which is
...
finitely meet-closed core by proposition \ref{f-meet-closed}, with
...
Then (taking into account the proposition \ref{bool-compl})
\[
...
Let $\alpha\in\card T$. Then (because $\card T$ is limit ordinal,
...
$\Cor a\in\down a$ and $\Cor b\in\down b$ by the corollary \ref{cor-less}
...
\bigsqcap^{\mathfrak{Z}}\setcond{x\sqcup^{\mathfrak{Z}}y}{x\in\up a,y\in\up b}=\top & \Leftrightarrow\\
...
complete lattice filtrator with down-aligned, finitely meet-closed,
...
is a filtrator with join-closed core by corollary \ref{semifilt-joinclosed}.
...
\item \label{compl-eq-dual-conc}$a^{\ast}=a^{+}=\overline{\Cor a}$ for
...
finitely meet-closed by proposition \ref{f-meet-closed}, with separable
...
\end{proof}
\begin{cor}
For powerset filtrators $a^{\ast}=a^{+}=\overline{\Cor a}=\overline{\Cor'a}\in\mathfrak{Z}$.\end{cor}
\begin{defn}
...
filtrator.
\item \label{cor-join-atom-f}$(\mathfrak{A};\mathfrak{Z})$ is a filtrator
...
theorem conditions it follows that $\Cor'(a\sqcap^{\mathfrak{A}}b)$
exists.
...
filtrator.
\item \label{cor-inf-meet-f}$(\mathfrak{A};\mathfrak{Z})$ is a filtrator
...
\item [{\ref{cor-inf-meet-f}$\Rightarrow$\ref{cor-inf-meet-fltr}}] Obvious.
...
filtrator over a complete atomistic lattice.
...
meet-close core by \ref{f-meet-closed}. $\mathfrak{A}$ is starrish
...
\ref{filt-is-complete}), with binarily meet-closed core (proposition
\ref{f-meet-closed}), with separable core (theorem \ref{when-sep-core}).
...
filtrator over a boolean lattice.
\item \label{intrs-compl-filt}$(\mathfrak{A};\mathfrak{Z})$ is a filtrator
down-aligned, with binarily meet-closed and separable core.
...
obvious \ref{filt-aligned}, proposition \ref{f-meet-closed}, theorem
\ref{when-sep-core}.
...
filtrator over a complete boolean lattice.
\item \label{join-compl-filt}$(\mathfrak{A};\mathfrak{Z})$ is a filtrator
up-aligned, with binarily join-closed and co-separable core.
...
obvious \ref{filt-aligned}, corollary \ref{semifilt-joinclosed},
...
and up-aligned finitely meet-closed and finitely join-closed distributive
...
filtrator is finitely meet-closed by the theorem \ref{up-filt-crit}
...
\begin{enumerate}
\item \label{bool-adj-f}$\mathfrak{A}$ is the poset of filters over a
boolean lattice $\mathfrak{Z}$.
\item \label{bool-adj-fltr}$(\mathfrak{A};\mathfrak{Z})$ is an up-aligned
finitely join-closed and finitely meet-closed distributive lattice
filtrator over a boolean lattice.
...
~
\begin{description}
\item [{\ref{bool-adj-f}$\Rightarrow$\ref{bool-adj-fltr}}] Obvious.
...
\begin{enumerate}
\item \label{f-back-distr-p}$\mathfrak{A}$ is the poset of filters on
a set.
\item \label{f-back-distr-f}$\mathfrak{A}$ is the poset of filters over
a boolean lattice $\mathfrak{Z}$.
...
\item [{\ref{f-back-distr-f}$\Rightarrow$\ref{f-back-distr-fltr}}] Obvious.
...
and with join-closed core (corollary \ref{semifilt-joinclosed}).
...
We have shown that $X\in(\mathfrak{A})$.
...
\item [{\ref{fs-fclos}}] We have $T\subseteq S$ and need to prove that
...
(used theorem \ref{up-filt-crit}) and $\mathcal{X}\sqcup^{\mathfrak{A}}\mathcal{Y}=(X\sqcup^{\mathfrak{A}}\overline{X})\sqcap^{\mathfrak{A}}\mathcal{A}=(X\sqcup^{\mathfrak{Z}}\overline{X})\sqcap^{\mathfrak{A}}\mathcal{A}=\top^{\mathfrak{A}}\sqcap^{\mathfrak{A}}\mathcal{A}=\mathcal{A}$
...
We have
\[
\mathcal{X}=\mathcal{X}\sqcup(X\sqcap^{\mathfrak{A}}\mathcal{Y})=\mathcal{X}=X\sqcap^{\mathfrak{A}}(\mathcal{X}\sqcap^{\mathfrak{A}}\mathcal{Y})=X\sqcap^{\mathfrak{A}}\mathcal{A}.
\]
...
\]
(used propositions \ref{compl-eq-dual}, \ref{dual-cor-meet}).
\end{description}
\end{proof}
...
\begin{align*}
\exists A\in\mathcal{A}:X\sqcap^{\mathfrak{Z}}A=Y\sqcap^{\mathfrak{Z}}A & \Leftrightarrow\\
\exists A\in\mathcal{A}:\uparrow X\sqcap^{\mathfrak{A}}\uparrow A=\uparrow Y\sqcap^{\mathfrak{A}}\uparrow A & \Rightarrow\\
\exists A\in\mathcal{A}:\uparrow X\sqcap^{\mathfrak{A}}\uparrow A\sqcap^{\mathfrak{A}}\mathcal{A}=\uparrow Y\sqcap^{\mathfrak{A}}\uparrow A\sqcap^{\mathfrak{A}}\mathcal{A} & \Leftrightarrow\\
\exists A\in\mathcal{A}:\uparrow X\sqcap^{\mathfrak{A}}\mathcal{A}=\uparrow Y\sqcap^{\mathfrak{A}}\mathcal{A} & \Leftrightarrow\\
...
\exists A_{0},A_{1}\in\mathcal{A}:X\sqcap^{\mathfrak{Z}}A_{0}=Y\sqcap^{\mathfrak{Z}}A_{1} & \Rightarrow\\
\exists A_{0},A_{1}\in\mathcal{A}:X\sqcap^{\mathfrak{Z}}A_{0}\sqcap^{\mathfrak{Z}}A_{1}=Y\sqcap^{\mathfrak{Z}}A_{0}\sqcap^{\mathfrak{Z}}A_{1} & \Rightarrow\\
\exists A\in\mathcal{A}:Y\sqcap^{\mathfrak{Z}}A=X\sqcap^{\mathfrak{Z}}A.
\end{align*}
...
call $\sim$ a congruence on $A$ when $(\sim)\bigcap A\times A$
...
$\dom\mathfrak{A}$. We will define order of families of posets by
the formula 
\[
...
this new poset $\mathfrak{A}=\prod\mathfrak{A}$ \emph{the function
...
\[
\bot^{\mathfrak{A}}=\prod_{i\in\dom\mathfrak{A}}\bot^{\mathfrak{A}_{i}}.
\]
\end{obvious}
\begin{prop}
...
\exists i\in\dom\mathfrak{A},c\in\mathfrak{A}_{i}\setminus\{\bot^{\mathfrak{A}_{i}}\}:(c_{i}\sqsubseteq a_{i}\land c_{i}\sqsubseteq b_{i}) & \Leftrightarrow\\
...
\begin{cor}
If $\mathfrak{A}_{i}$ are lattices then $\mathfrak{A}$ is a lattice.\end{cor}
\begin{obvious}
If $\mathfrak{A}_{i}$ are distributive lattices then $\mathfrak{A}$
...
Take $y=(((\dom\mathfrak{A})\setminus\{i\})\times\{0\})\cup\{(i;x)\}$.
...
is (finitely) join-closed if every $(\mathfrak{A}_{i};\mathfrak{Z}_{i})$
is (finitely) join-closed.
\item The filtrator $\left(\prod\mathfrak{A};\prod\mathfrak{Z}\right)$
is (finitely) meet-closed if every $(\mathfrak{A}_{i};\mathfrak{Z}_{i})$
is (finitely) meet-closed.
\end{enumerate}
\end{prop}
\begin{proof}
Let every $(\mathfrak{A}_{i};\mathfrak{Z}_{i})$ be finitely join-closed.
...
Take $y=((n\setminus\{i\})\times\{0\})\cup\{(i;x)\}$. Then we have
...
\text{(because every \ensuremath{\mathfrak{Z}_{i}} is nonempty)}\Leftrightarrow\forall i\in\dom\mathfrak{A}_{i}:Z(\mathfrak{A}_{i})=\mathfrak{Z}_{i}.
\end{multline*}
\end{proof}
\begin{prop}
...
are $n$-indexed families of distributive lattices with least elements
...
\begin{defn}
\index{filter!Fréchet}\index{filter!cofinite}$\Omega=\setcond{\mathfrak{U}\setminus X}{X\text{ is a finite subset of }\mathfrak{U}}$
is called either \emph{Fréchet filter} or \emph{cofinite filter}.
\end{defn}
...
but $y\sqcap1\ne\bot$ consequently the lattice is not with separable
center.

In this section $\mathfrak{A}$ denotes the set of filters on a set.\end{proof}
\begin{example}
...
\begin{proof}
Let $a=\Delta$ and $S=\rsupfun{\uparrow}\setcond{(\epsilon;+\infty)}{\epsilon>0}$.
...
Let take $U=\mathbb{N}$ and $\mathcal{A}=\Omega$ be the Fréchet
filter on $\mathbb{N}$.
