...
\fxerror{It is not necessarily a filter as it may be the empty
set. The below in this section is thus erroneous (however it
seems that the errors are not very serious and can all be
corrected).}
\end{lem}
...
\begin{proof}
$\mathcal{A}\div B =
\mathscr{S}\mathcal{A}\cap\subsets B =
\mathscr{S}\mathcal{B}\cap\subsets B =
\mathcal{B}\div B$.
\end{proof}
