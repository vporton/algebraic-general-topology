...
\begin{thm}\label{pdiff-eq2}
  Let $(\mathfrak{A}; \mathfrak{Z})$ be a complete co-brouwerian atomistic
  lattice filtrator with binarily meet-closed and separable boolean
  core. Then the three expressions of pseudodifference in theorem~\ref{pdiff-eq1} are also equal to
...
\begin{prop}
For filters on boolean lattices, the three expressions of the pseudodifference of filters~$a$ and~$b$
in theorem~\ref{pdiff-eq1} are also equal to
  $\bigsqcup \setcond{ a \sqcap \overline{B} }{ B \in \up b }$.
\end{prop}

\begin{proof}
We can apply theorem~\ref{pdiff-eq2} because
...
\item with meet-closed core by corollary~\ref{f-meet-closed}.
\end{itemize}
\end{proof}
