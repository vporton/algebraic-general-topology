...
\begin{example}
An $\mathbb{R}^{n-1}$ subspace of a~$\mathbb{R}^n$ ($n\geq 1$) euclidean
space has exactly two surface sides.
\end{example}
...
\fxwarning{Check if $(\rsupfun{\mu}\{a\})\setminus T$ has two connected
components if~$T$ is a plane in~$\mathbb{R}^3$.}
