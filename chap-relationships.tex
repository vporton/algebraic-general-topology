...
\index{funcoid!induced by reloid}We will define funcoid $\tofcd f$ induced by reloid~$f$ (that is
a function $\tofcd:\mathsf{RLD}(A;B)\rightarrow\mathsf{FCD}(A;B)$ for every sets~$A$,~$B$). But first we define
a little more general case of $\tofcd f:\mathsf{RLD}\sharp(A;B)\rightarrow\mathsf{FCD}(A;B)$ for given sets~$A$ and~$B$
(recall that $\mathsf{RLD}(A;B)$ is a special case of $\mathsf{RLD}\sharp(A;B)$).

$\tofcd f\in\mathsf{FCD}(A;B)$ is defined by the formulas
...
Now define $\tofcd f\in\mathsf{FCD}(\Src f;\Dst f)$ for every $f\in\mathsf{RLD}(\Src f;\Dst f)$ (that is for a reloid~$f$ with specific source and destination)
by the above formulas with sets~$A$, $B$ taken as~$\Src f$,~$\Dst f$,
with $\mathsf{RLD}(\Src f;\Dst f)$ taken for $\mathsf{RLD}\sharp(\Src f;\Dst f)$)
(recall that $\mathsf{RLD}(\Src f;\Dst f)$ is the base of a primary filtrator over $\mathbf{Rel}(\Src f;\Dst f)$ and
$\mathsf{RLD}\sharp(\Src f;\Dst f)$ can be taken as the base of an arbitrary primary filtrator over $\mathbf{Rel}(\Src f;\Dst f)$).

\begin{obvious}
$\Src\tofcd f=\Src f$, $\Dst\tofcd f=\Dst f$ for every reloid~$f$.
\end{obvious}
