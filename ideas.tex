...
Equivalence of being co-brouwerian and existence of
the adjoint of $a\sqcup$ without the requirement to be
complete.

Let $C$ be a small category whose objects are sets
(generalize). $C'$ is the semigroup whose elements are
$\setcond{\uparrow^{(A,B)}f}{A\in\up\dom f,B\in\up\im f}$
for $f\in\Hom_C$.
Use rectangular embedding from \texttt{volume-2.pdf}.
$\im$ and~$\dom$ can be defined as the sets of sets, composition with $\mathscr{E}$ for which is injective.
Also equate filters of different bases.
This way we can eliminate using typed sets.

``Unfixed'' for more general settings than filters on sets
(and categories~$\mathsf{FCD}$,~$\mathsf{RLD}$).

Which filter operations are congruences on
equivalence of filters?

Properties of the filtrator of small unfixed filters
and small sets.
