...
\emph{Ategory} is a function $\Ob\times\Ob \rightarrow \Hom$. It differs from a category that composition is undefined and it is not always
possible to determine the source and destination object knowing a morphism.
Consider $\mathsf{FCD}[A;B]$ and $\mathsf{RLD}[A;B]$. Call this \emph{ategory} (category without ``composition'').
This has the advantage that $\mathsf{RLD}[A;B]$ can be taken as the base of \emph{arbitrary} primary filtrator over $A\times B$.
However we need only $\mathbf{Rel}[A;B]$, $\mathsf{RLD}[A;B]$, and $\Gamma[A;B]$, but there is no use of $\mathsf{FCD}[A;B]$.
Moreover, there is no need to define ategories, just use particular cases.
