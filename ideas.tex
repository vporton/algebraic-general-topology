...
If r is left adjoint to i, we have Hom(A, i(X x Y)) = Hom(r(A), X x Y) = Hom(r(A), X) x Hom(r(A), Y) = Hom(A, i(X)) x Hom(A, i(Y)).
|| If also the left adjoint is full and faithful:
Hom(A, i(r(X) x r(Y))) = Hom(r(A), r(X) x r(Y)) = Hom(r(A), r(X)) x Hom(r(A), r(Y)) = Hom(A, X) x Hom(A, Y)
...
Is subatomic product this?

Subspace topology for space~$\mu$ and set~$X$ is equal to $\mu\sqcap(X\times^{\mathsf{FCD}} X)$.

Change terminology: \emph{montone} $\rightarrow$ \emph{increasing}.
...
\url{ftp://ftp.math.ethz.ch/EMIS/journals/IJMMS/Volume32_7/239107.pdf}

$g\sqsubseteq f^{\circ}\circ h \Leftrightarrow f\circ g\sqsubseteq h$?

$\lim_{x\rightarrow a} f(x) = b$ iff $x\rightarrow a$ implies $\supfun{f}x\rightarrow b$ for all filters~$x$.
