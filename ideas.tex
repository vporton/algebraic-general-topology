...
\emph{Surface side} of a set~$T$ is a connected component (regarding $\mu$ or
...
and closed disk has one surface side. We can also define \emph{special point}
as a limit point of a subset of~$T$ such that all elements~$x$ of the subset
have the same cardinality of connected components of
$(\rsupfun{\mu}\{x\})\setminus T$ but the the limit point has a different
cardinality. Excluding special points from closed disk produces
open disk (prove!) Alternatively try it to define using limit in proximity
of filters. Maybe such proximity can be defined (for a filter~$x$) similarly to:
\[
\{x\}\mathrel{\delta}Y \Leftrightarrow
\forall X\in\up x\exists y\in Y: X\in\up y.
\]
