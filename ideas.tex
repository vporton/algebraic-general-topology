...
Generalized limits on~$a$ are bijective with the sets of all ultralimits on~$\atoms a$.

Which filter operations are congruences on
equivalence of filters?

It seems that for normed vector spaces (generalize!) generalized limit is defined (up to isomorphism) by the following axioms:
\begin{enumerate}
\item The results also form a vector space.
\item It termines the ``customary'' limit.
\item Limit is zero iff (can be replaced with one-side implication?) the function is
  restricted at a filter.
\end{enumerate}

\section{Misc}

A space if compact iff $c\circ a$, where~$c$ is the core part funcoid, is entirely defined.

Axiomatic definition of generalized limit (like: generalized limit is a value an algebraically closed set
from which the customary limit or its absence can be restored).
...
\url{https://www.researchgate.net/publication/347963563_Weak_completions_of_paratopological_groups}
