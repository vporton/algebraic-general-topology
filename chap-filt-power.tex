...
\begin{thm}
  ...
  \begin{itemize}
    ...
    \item $S \mapsto \bigsqcap^{\mathsf{RLD}} S = \left(\bigsqcap^{\mathsf{RLD}} S\right)|_{\mathcal{X}}$ if $\card B\ne 1$ or \\
      $S=\{s\} \mapsto \left(\bigsqcap^{\mathsf{RLD}} S\right)|_{\mathcal{X}} = s|_{\mathcal{X}}$ if $\card B=1$.
  \end{itemize}
\end{thm}
