...
a union of powersets closed on small unions.
\end{defn}
...
\begin{defn}
I will call an \emph{almost funcoid} a pointfree funcoid between almost universes.
\end{defn}

Almost funcoids encompass both funcoids and unfixed funcoids
(because unfixed funcoids bijectively correspond to pointfree
funcoids on a Grotendieck universe).

\begin{defn}
I will denote $\mathsf{aFCD}$ the category of almost funcoids (the wide subcategory of the category of pointfree funcoids whose objects are almost universes).
\end{defn}
...
\begin{enumerate}
\item $f\circ(g\sqcup h)=f\circ g\sqcup f\circ h$ for $g,h\in\mathsf{aFCD}(A,B)$,
$f\in\mathsf{aFCD}(B,C)$;
\item $(g\sqcup h)\circ f=g\circ f\sqcup h\circ f$ for
$g,h\in\mathsf{aFCD}(B,C)$,
$f\in\mathsf{aFCD}(A,B)$.
\end{enumerate}
\end{lem}
...
\fxnote{Also prove for unfixed funcoids (first define~$\up$).}
