...
Let $U$ be a set.
\begin{defn}
\index{path}\emph{Path} between two elements $a,b\in U$ in a set
$A\subseteq U$ through binary relation $\mu$ is the finite sequence
...
\begin{prop}
There exists path between every element $a\in U$ and that element
itself.\end{prop}
\begin{proof}
...
for some $X,Y\in\mathscr{P}U\setminus\{\emptyset\}$ such that
