...
\begin{obvious}
Micronization is always irreflexive.
\end{obvious}

\begin{proof}
In the formula $\mu (E) = \bigsqcap^{\mathsf{RLD}} \setcond{ f \in
\mathbf{Rel} }{ S (f) \sqsupseteq E}$ we can replace every $f$ with
$f \setminus 1$ because $S (f)
\sqsupseteq E \Rightarrow S (f \setminus 1) \sqsupseteq E$. So we get:
\[
  \mu (E) = \bigsqcap^{\mathsf{RLD}} \setcond{ f\setminus 1 }{ f \in
  \mathbf{Rel}, S (f) \sqsupseteq E} \sqsubseteq \top\setminus 1.
\]
Thus $\mu (E)$ is irreflexive.
\end{proof}

\fxerror{Wrong ``reflexive'' instead of irreflexive in this section below.}
