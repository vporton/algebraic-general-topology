...
\end{enumerate}
\fxnote{$\id^{\mathcal{C}(\Src f,\Src f)}_{[X]\sqcap[\Src f]} = f \to
\id^{\mathcal{C}(\Src f)}_{X\sqcap\Src f}$? It is defined below.}
\end{defn}
...
\fxwarning{$\id^{\mathcal{C}(\Dst f)}_X$ is defined below. Also for $\operatorname{Im}$.}
\end{prop}
...
\begin{cor}
$\operatorname{dom} (g \circ f) \sqsubseteq \operatorname{dom} f$, $\operatorname{im} (g \circ f)
\sqsubseteq \operatorname{im} g$ whenever $\operatorname{dom}$/$\operatorname{im}$ are defined.
\end{cor}
...
\fxnote{It seems this can be defined not only for funcoids, using image/domain.}
\end{defn}
