...
\fxerror{The left side is between unfixed filters,
buth the right is between filters on small sets.
So, it's wrong.}
\end{prop}
...
Thus if $\mathcal{P}\nasymp\mathscr{S}X$ then
(by the lemma)
$\mathcal{P}\sqcap\mathscr{S}A\nasymp\mathscr{S}X$;
$\supfun{\mathscr{S}(X\times Y)}\mathcal{X}=
[\supfun{X\times_{A,B}Y}(\mathcal{P}\div A)]=$
??
\end{proof}
