...
\section{Alternative way to define unfixed morphisms}

Consider more general axiomatic way to define unfixed
morphisms:

Let we have a category~$\mathcal{C}$ whose objects are
elements of a lattice (or poset?)~$\mathfrak{A}$. It may
be the lattice of small sets or of small filters or of
unfixed filters.

Define $\mathcal{E}^{A,B}$ axiomatically or better define
$\id^{\mathcal{C}(A,B)}_X$:

\begin{enumerate}
\item $\id^{\mathcal{C}(A,B)}_X\in\Hom_{\mathcal{C}}(A,B)$
whenever $X\sqsubseteq A\sqcap B$;
