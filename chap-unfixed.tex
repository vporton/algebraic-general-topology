...
\begin{prop}
$\operatorname{IM} f = \mathscr{S}\operatorname{Im} f$;
$\operatorname{dom} f = \mathscr{S}\operatorname{Dom} f$.
\end{prop}
...
$\operatorname{IM} f = \setcond{Y \in \mathfrak{Z}}{\id^{\mathcal{C}(\Dst f,\Dst f)}_{Y\sqcap\Dst f} \circ f = f} = \mathscr{S}\operatorname{Im} f$. \fxwarning{More detailed proof.}
\end{proof}
...
~
\begin{enumerate}
\item $\operatorname{Im} f = \setcond{Y \in \subsets\Dst f}{
  \mathcal{E}_{\mathcal{C}}^{\Dst f, Y} \circ f \sim f}$;
\item $\operatorname{Dom} f = \setcond{X \in \subsets\Src f}{
