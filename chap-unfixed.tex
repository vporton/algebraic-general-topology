...
\begin{cor}\label{iota-emb}
  ...
  \fxerror{I erroneously use below that $\iota_{B_0, B_1} |_{f \in \Hom_{\mathcal{C}} (A_0, A_1)}$ is an order isomorphism
  without a proof.}
\end{cor}
...
Now we can apply that $x\mapsto[x]$ is an order embedding:
$\bigsqcap S=
\left[\bigsqcap\setcond{F\sqcap\iota_{A,B}G}{g\in S,G\in g}\right]$. We have provided an explicit formula for
$\bigsqcap S$. \fxerror{Being order embedding is not enough,
needs isomorphism.}
\end{proof}
