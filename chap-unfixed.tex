...
\section{Rebase of unfixed filters}
...
\fxwarning{This generalizes a theorem in the book.}
\end{prop}
...
The proposition~\ref{sim-rebase} allows to define:
...
\begin{obvious}
$\mathcal{A}\div B = \mathscr{S}\mathcal{A}\cap\subsets B$
for every unfixed filter~$\mathcal{A}$.
\fxnote{It can be taken as the definition of rebase.}
\fxnote{It also holds for filter~$\mathcal{A}$.}
\end{obvious}
...
filters~$\mathcal{A}$ and~$\mathcal{B}$ then
...
\begin{xca}
$(a\div B)\sqcap(b\div B) = (a\sqcap b)\div B$ for every
filters~$a$ and~$b$ of the same base~$C$ and
set~$B\sqsupseteq C$.
\end{xca}
...
We will prove $\bigsqcup S = \left[\bigsqcup_{\mathcal{X}\in S}(a_{\mathcal{X}}\div Z)\right]$.

Let first prove that for every $\mathcal{K}\in S$ we have
$\mathcal{K} \sqsubseteq \left[\bigsqcup_{\mathcal{X}\in S}(a_{\mathcal{X}}\div Z)\right]$.
We have $a_{\mathcal{K}}\div Z\sqsubseteq\bigsqcup_{\mathcal{X}\in S}(a_{\mathcal{X}}\div Z)$
