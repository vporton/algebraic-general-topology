...
\begin{prop}
$f\mapsto\iota_{A_1,B_1}f$ is an order embedding from
$\mathcal{C}(A_0,B_0)$ to $\mathcal{C}(A_1,B_1)$
if $A_0\sqsubseteq A_1\land B_0\sqsubseteq B_1$.
\end{prop}

\begin{prop}
If every $\Hom$-set is a lattice, then the poset of
unfixed morphism is a lattice.
\fxnote{What about semilattices?}
\fxnote{We also can prove this way that $\mathcal{E}$
and $\id$ are weakly meta-complete.}
\end{prop}
