...
\begin{proof}
(TODO: check)
Let $\mathcal{X}$ and~$\mathcal{Y}$ are equivalent.
It is enough (??) to prove that
$\Base(\mathcal{X})\cap\Base(\mathcal{Y})\in\mathcal{X}$
(and similarly for~$\mathcal{Y}$).
Suppose for the contrary that
$\Base(\mathcal{X})\cap\Base(\mathcal{Y})\notin\mathcal{X}$, that is
$\Base(\mathcal{Y})\notin\mathcal{X}$. But then
$\Base(\mathcal{Y})=\mathcal{X}\div\Base(\mathcal{Y})
\notin\mathcal{Y}$ what is?? impossible.
\end{proof}
...
~
\begin{description}
\item[Reflexivity] From proposition~\ref{rebase-itself}.
\item[Simmetry] Obvious.
\item[Transitivity] Let $\mathcal{X}\sim\mathcal{Y}$ and
$\mathcal{Y}\sim\mathcal{Z}$. Then
$\mathcal{Y} = \mathcal{X} \div \Base(\mathcal{Y})$ and
$\mathcal{Z} = \mathcal{Y} \div \Base(\mathcal{Z})$.
So $\mathcal{Z} = (\mathcal{X} \div \Base(\mathcal{Y})) \div \Base(\mathcal{Z})$.
??
\end{description}
