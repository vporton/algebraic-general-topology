...
\fxerror{$\mathcal{X}\div B=
\mathscr{S}\mathcal{X}\cap\subsets B=
\rsupfun{B\cap}\mathscr{S}\mathcal{X}$. The first equality does not hold for $B\notin\mathscr{S}\mathcal{X}$ (how to depict this
on the diagram?).}
...
\begin{lem}
$\mathscr{S}$ and $\mathcal{X}\mapsto\mathcal{X}\cap\subsets B$
...
\begin{proof}
First, $\mathscr{S}x\in\setcond{\mathcal{X}\in\mathfrak{F}(\text{small sets})}{B\in\mathcal{X}}$
because of theorem~\bookref{mathscrs-iso} (TODO: Formulate it
in that theorem fully.)
...
\fxnote{Can we eliminate some propositions as easy consequences of this?}
...
\setcond{\mathcal{X}\in\mathfrak{F}(\text{small sets})}
  {B\in\mathcal{X}}
\arrow[ru, shift left, "\mathcal{X}\mapsto\mathcal{X}\cap\subsets B=\rsupfun{B\cap}\mathcal{X}"]
\arrow[rr, shift left]
...
\begin{prop}
\fxwarning{This is a consequences of the theorem with diagram.}
