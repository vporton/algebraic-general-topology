...
set~$B\supseteq C$.
\fxnote{What about an infinite set here? Also for join.}
\fxwarning{Remove this exercise as a special case of the
above.}
\end{xca}
...
$\Base(a)=\Base(b)$.
\fxnote{Generalize this proposition for an (infinite)
set of filters.}
\end{prop}
...
\fxnote{It holds in more general case if the set (not
necessarily small) has an upper bound (the bound being
a set of an unfixed filter are both OK).}
\end{thm}
...
\begin{cor}
Join of two unfixed filters always exists.
\end{cor}
