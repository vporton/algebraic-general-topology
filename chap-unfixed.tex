...
\begin{prop}
~\fxerror{Wrong in details.}
\begin{enumerate}
\item $\mathscr{S}$ from a $\Hom$-set $\mathsf{FCD}(A,B)$
is an order embedding and a functor.
\item $\mathscr{S}$ from unfixed funcoids is an order embedding and a functor (=~semigroup homomorphism).
\end{enumerate}
\end{prop}
