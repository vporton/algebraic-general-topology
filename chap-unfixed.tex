...
Then $\id^{\mathcal{C}(\Dst f,\Dst f)}_{Y\sqcap\Dst f} = \id^{\mathcal{C}(\Dst f,\Dst f)}_{Y'\sqcap\Dst f}$;

$\operatorname{Im} f = \setcond{Y \in \mathfrak{Z}}{\id^{\mathcal{C}(\Dst f,\Dst f)}_{Y\sqcap\Dst f} \circ f = f} = \mathscr{S}\im f$. \fxwarning{More detailed proof.}
\end{proof}
