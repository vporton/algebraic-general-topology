...
\begin{enumerate}
\item $X\times_{A,B}Y=
\max_{T\in\mathcal{C}(A,B)}(\id^{\mathcal{C}(A,A)}_X\circ T\circ\id^{\mathcal{C}(A,A)}_Y)$;
\item $\id^{\mathcal{C}(A,A)}_X\circ f\circ\id^{\mathcal{C}(A,A)}_Y=f\sqcap(X\times_{A,B}Y)$ for every
...
If $\id_Y\circ f\circ\id_X=f\sqcap(X\times Y)$ for some
function~$\times$ of two arguments then
restricted identities are metamonovalued and metainjective.
