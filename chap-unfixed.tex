...
For a partially ordered category we can introduce additional axiom $X\sqsubseteq Y\Rightarrow
\id^{\mathcal{C}(A,B)}_X\sqsubseteq\id^{\mathcal{C}(A,B)}_Y$. \fxnote{Do we need this axiom?}

For dagger categories we can introduce additional axiom
$(\id^{\mathcal{C}(A,B)}_X)^\dagger =
\id^{\mathcal{C}(A,B)}_X$.

Then define $\mathcal{E}_{\mathcal{C}}^{A,B} =
...
\begin{prop}
  ...
  \begin{enumerate}
    ...
    \item If $A \sqsupseteq B$ then $\mathcal{E}_{\mathcal{C}}^{A,B}$ is a
    epimorphism.
  \end{enumerate}
\end{prop}
...
\begin{prop}
  ...
  iff $B \sqsupseteq A \sqcap C$ (for every sets $A$, $B$, $C$).
\end{prop}
...
\begin{cor}
  If $f : X \rightarrow Y$ is a morphism of
...
\begin{prop}
  ...
  \subseteq B_1$. Then $\iota_{B_0, B_2} (g \circ f) = \iota_{B_1, B_2} g
...
We can define $\id^{\mathcal{C}}_X=\uparrow(A,B,\id_X)$.
Note that $\mathcal{E}$ is a special case of $\id^{\mathcal{C}}_X$.
...
~
\begin{enumerate}
\item $\im f = \setcond{Y \in \subsets\Dst f}{\mathcal{E}_{\mathcal{C}}^{Y, \Dst f} \circ \mathcal{E}_{\mathcal{C}}^{\Dst f,
Y} \circ f = f}$;
\item $\dom f = \setcond{X \in \subsets\Src f}{f\circ\mathcal{E}_{\mathcal{C}}^{\Src f, X} \circ \mathcal{E}_{\mathcal{C}}^{X, \Src f} = f}$.
\end{enumerate}
\end{defn}
...
~
\begin{enumerate}
\item $\operatorname{Im} f = \setcond{Y \in \Ob
(\mathcal{C})}{\mathcal{E}_{\mathcal{C}}^{Y, \Dst f} \circ \mathcal{E}_{\mathcal{C}}^{\Dst f,
Y} \circ f = f}$;
\item $\operatorname{Dom} f = \setcond{X \in X \in \Ob (\mathcal{C})}{f\circ\mathcal{E}_{\mathcal{C}}^{\Src f, X} \circ \mathcal{E}_{\mathcal{C}}^{X, \Src f} = f}$.
\end{enumerate}
\end{defn}

\begin{prop}
$\operatorname{Im} f = \mathscr{S}\im f$;
$\operatorname{Dom} f = \mathscr{S}\dom f$.
\end{prop}

\begin{proof}
Define $Y' = Y \cap \Dst f$ for every $Y \in \Ob (\mathcal{C})$.

Then $\mathcal{E}_{\mathcal{C}}^{Y, \Dst f} \circ
\mathcal{E}_{\mathcal{C}}^{\Dst f, Y} = \uparrow (\mathcal{E}^{Y,
\Dst f} \circ \mathcal{E}^{\Dst f, Y}) = \uparrow
(\mathcal{E}^{Y', \Dst f} \circ \mathcal{E}^{\Dst f, Y'})
=\mathcal{E}_{\mathcal{C}}^{Y', \Dst f} \circ
\mathcal{E}_{\mathcal{C}}^{\Dst f, Y'}$;

$\operatorname{Im} f = \setcond{Y \in \Ob
(\mathcal{C})}{\mathcal{E}_{\mathcal{C}}^{Y, \Dst f} \circ
\mathcal{E}_{\mathcal{C}}^{\Dst f, Y} \circ f = f} = \mathscr{S}
\setcond{Y' \in \subsets \Dst f}{\mathcal{E}_{\mathcal{C}}^{Y,
\Dst f} \circ \mathcal{E}_{\mathcal{C}}^{\Dst f, Y} \circ f = f}$.
\end{proof}

\begin{conjecture}
$\im f$ may be not a filter for ??.
\end{conjecture}

\begin{prop}\label{dst-in-im}
$\Dst f\in\im f$; $\Src f\in\dom f$ for ??.
\end{prop}

\begin{proof}
Prove $\Dst f\in\im f$ (the other is similar):
We need to prove that $\mathcal{E}_{\mathcal{C}}^{\Dst f, \Dst f} \circ \mathcal{E}_{\mathcal{C}}^{\Dst f,
\Dst f} \circ f = f$ but it follows from
$\mathcal{E}_{\mathcal{C}}^{\Dst f, \Dst f}=
\uparrow\mathcal{E}^{\Dst f, \Dst f} =
\uparrow 1^{\mathbf{Rel}} = 1^{\mathcal{C}}$.
\end{proof}
...
\begin{proof}
  ...
  \circ \mathcal{E}^{\Dst f, Y} = (\Dst f, B, \id_{Y \cap
  \Dst f \cap B}) = (Y, B, \id_{Y \cap B}) \circ (\Dst f, Y,
  \id_{Y \cap \Dst f}) =\mathcal{E}^{Y, B} \circ
...
\[\iota_{A_0\cup A_1,B_0\cup B_1}f=\iota_{A_0\cup A_1,B_0\cup B_1}g.\]
\end{defn}
...
\item[$\Rightarrow$]
$\iota_{A_0\cup A_1,B_0\cup B_1}f=\iota_{A_0\cup A_1,B_0\cup B_1}g$;
$\iota_{A_1,B_1}\iota_{A_0\cup A_1,B_0\cup B_1}f=\iota_{A_1,B_1}\iota_{A_0\cup A_1,B_0\cup B_1}g$;
$\iota_{A_1,B_1}f=\iota_{A_1,B_1}g$;
$\iota_{A_1,B_1}f=g$. $\iota_{A_0,B_0}g=f$ is similar.

\item[$\Leftarrow$] $\iota_{A_1,B_1}f=g$ implies
$A_1\in\im g$ and $A_0\in\im g$.
Thus?? $A_0\cap A_1\in\im g$.
Consequently??
$\iota_{A_0\cup A_1,B_0\cup B_1}g=
\iota_{A_0\cup A_1,B_0\cup B_1}\iota_{A_1,B_1}f=
\iota_{A_0\cup A_1,B_0\cup B_1}f$.
\end{description}
\end{proof}
...
Then
$\iota_{A_0\cup A_1,B_0\cup B_1}f=\iota_{A_0\cup A_1,B_0\cup B_1}g$ and
$\iota_{A_1\cup A_2,B_1\cup B_2}g=\iota_{A_1\cup A_2,B_1\cup B_2}h$.

Thus
\[\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}\iota_{A_0\cup A_1,B_0\cup B_1}f=\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}\iota_{A_0\cup A_1,B_0\cup B_1}g\] and
\[\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}\iota_{A_1\cup A_2,B_1\cup B_2}g=\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}\iota_{A_1\cup A_2,B_1\cup B_2}h\]
that is (propositon~\ref{two-iotas})
\[\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}f=\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}g\]
and
\[\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}g=\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}h.\]
Combining,
$\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}f=\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}h$ and thus
\[\iota_{A_0\cup A_2,B_0\cup B_2}\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}f=\iota_{A_0\cup A_2,B_0\cup B_2}\iota_{A_0\cup A_1\cup A_2,B_0\cup B_1\cup B_2}h;\]
(again propositon~\ref{two-iotas}) $\iota_{A_0\cup A_2,B_0\cup B_2}f=\iota_{A_0\cup A_2,B_0\cup B_2}h$
that is $f\sim h$.
\end{description}
\end{proof}
...
morphisms
$f'=\iota_{\Src f,\Dst f\cup\Src g}f\sim f$ and
$g'=\iota_{\Dst f\cup\Src g,\Dst g}g\sim g$.
...
We have
\begin{multline*}
\iota_{B_0\cup B_1,C_0\cup C_1}g_0 \circ
\iota_{A_0\cup A_1,B_0\cup B_1}f_0 = \\
\mathcal{E}_{\mathcal{C}}^{C_0,C_0\cup C_1}\circ g_0 \circ
\mathcal{E}_{\mathcal{C}}^{B_0\cup B_1,B_0}\circ\mathcal{E}_{\mathcal{C}}^{A_0,B_0\cup B_1}\circ
f_0\circ\mathcal{E}_{\mathcal{C}}^{A_0\cup A_1,B_0} = \\
= \text{(proposition~\ref{dst-in-im})} =
\mathcal{E}_{\mathcal{C}}^{C_0,C_0\cup C_1}\circ g_0 \circ
f_0\circ\mathcal{E}_{\mathcal{C}}^{A_0\cup A_1,B_0} = \\
\iota_{A_0\cup A_1,C_0\cup C_1}(g_0\circ f_0).
\end{multline*}
Similary
\[\iota_{B_0\cup B_1,C_0\cup C_1}g_1 \circ
\iota_{A_0\cup A_1,B_0\cup B_1}f_1 =
\iota_{A_0\cup A_1,C_0\cup C_1}(g_1\circ f_1).\]
...
thus having
$\iota_{A_0\cup A_1,C_0\cup C_1}(g_0\circ f_0) =
\iota_{A_0\cup A_1,C_0\cup C_1}(g_1\circ f_1)$ and so
$g_0\circ f_0\sim g_1\circ f_1$.
\end{proof}
...
~
\begin{enumerate}
\item $\im f = \setcond{Y \in \subsets\Dst f}{
  \mathcal{E}_{\mathcal{C}}^{\Dst f, Y} \circ f \sim f}$;
\item $\dom f = \setcond{X \in \subsets\Src f}{
  f\circ\mathcal{E}_{\mathcal{C}}^{X, \Src f} \sim f}$;
\item $\operatorname{Im} f = \setcond{Y \in \Ob(\mathcal{C})}{
  \mathcal{E}_{\mathcal{C}}^{\Dst f, Y} \circ f \sim f}$;
\item $\operatorname{Dom} f = \setcond{X \in \Ob(\mathcal{C})}{
  f\circ\mathcal{E}_{\mathcal{C}}^{X, \Src f} \sim f}$.
\end{enumerate}
\end{prop}

\begin{defn}
$\id^{\mathcal{C}}_A = [1_{\mathcal{C}}^A]$.
\fxwarning{Is it worth to introduce this notation?
Also take into account that it needs to be proved not
to conflict with identities of unfixed funcoids/reloids.}
\end{defn}
