...
\fxwarning{This section needs to be completely rewritten.}
...
\fxwarning{Need to prove that they really form
a category with restricted identities (and moreover
...
$\GR(\id^{\mathcal{C}(B,C)}_Y \circ \id^{\mathcal{C}(A,B)}_X) =
\GR((\id^{\mathsf{RLD}}_{Y\div(B\cap C)}
\div(B\times C)) \circ
(\id^{\mathsf{RLD}}_{X\div(A\cap B)}
\div(A\times B))) =
\GR((\id^{\mathsf{RLD}}_{Y\div(B\cap C)}
\div(B\times C)) \circ
\GR(\id^{\mathsf{RLD}}_{X\div(A\cap B)}
\div(A\times B))) =
((\GR\id^{\mathsf{RLD}}_{Y\div(B\cap C)}
\cap\subsets(B\times C)) \circ
(\GR\id^{\mathsf{RLD}}_{X\div(A\cap B)}
\cap\subsets(A\times B)) =
\text{(exercise)} =
(\GR\id^{\mathsf{RLD}}_{Y\div(B\cap C)}\circ
\GR\id^{\mathsf{RLD}}_{X\div(A\cap B)})
\cap\subsets(A\times C) =
\GR(\id^{\mathsf{RLD}}_{Y\div(B\cap C)}\circ
\id^{\mathsf{RLD}}_{X\div(A\cap B)})
\div(A\times C) = ??
$
\fxerror{$\id^{\mathsf{RLD}}_{Y\div(B\cap C)}\circ
\id^{\mathsf{RLD}}_{X\div(A\cap B)}$ is a nonsense.}
...
\fxnote{Unfixed reloids can be embedded into reloids
