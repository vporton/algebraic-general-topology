...
that~$Y\subseteq Q$. So $X\sqcap Y\in\mathcal{A}$
...
\begin{multline*}
\mathcal{A}=(\subsets X\cap\mathcal{A})\cup\setcond{Y\in\subsets\Base(\mathcal{A})}{Y\sqsupseteq X}=\\
(\subsets X\cap\mathcal{B})\cup\setcond{Y\in\subsets\Base(\mathcal{B})}{Y\sqsupseteq X}=\mathcal{B}.
\end{multline*}
\end{proof}
...
$\rsupfun{B\sqcap}\mathcal{X}\in\mathfrak{F}(B)$ obviously.
...
It remains to apply lemma~\bookref{three-loop-lem} (taking into account the proof of theorem~\ref{mathscrs-iso}).
\end{proof}
...
\begin{proof}
Proposition~\bookref{f-atoms-join}.
\end{proof}
...
\begin{proof}
Theorem~\bookref{da-is-free-star}.
\end{proof}
...
\begin{proof}
Theorem~\bookref{dual-cor-inf-meet}.
\end{proof}
...
\begin{proof}
Can be easily deduced from theorem~\bookref{dual-core-join}
and the triangular diagram (above) of isomorphic filtrators.
\end{proof}

\begin{conjecture}
The theorem~\bookref{closed-free-star} holds for unfixed
filters, too.
\end{conjecture}
