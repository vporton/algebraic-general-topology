...
\begin{prop}
The following axioms (holding for every $A,B\in\mathfrak{Z}$,
$\mathfrak{A}\ni X\sqsubseteq A$,
$\mathfrak{A}\ni Y\sqsubseteq B$,
$X\times_{A,B}Y\in\mathcal{C}(A,B)$) are equivalent:
\begin{enumerate}
\item $X\times_{A,B}Y=
\max_{T\in\mathcal{C}(A,B)}(\id^{\mathcal{C}(B,B)}_X\circ T\circ\id^{\mathcal{C}(A,A)}_Y)$;
\item $\id^{\mathcal{C}(B,B)}_X\circ f\circ\id^{\mathcal{C}(A,A)}_Y=f\sqcap(X\times_{A,B}Y)$ for every
morphism~$f\in\mathcal{C}(A,B)$.
\end{enumerate}
\end{prop}
...
\begin{defn}
The category \emph{with binary products} is a category with
restricted identities conforming to either of the above
axioms.
\end{defn}

\begin{prop}
All restricted identities are metamonovalued and metainjective
for every category with binary products.
\end{prop}
...
\fxnote{How to prove that $\iota_{A,B}|_{\Hom(X,Y)}$ is
...
\end{enumerate}
\fxnote{We also can prove this way that $\mathcal{E}$
and $\id$ are weakly meta-complete and weakly
meta-monovalued. This also concerns the identity
unfixed morphisms.}
\end{prop}
