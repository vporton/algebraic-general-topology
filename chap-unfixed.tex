...
definition may be replaced with a more general one.}
...
$f\sim X\times_{A_0,B_0}Y$ then
$f=X\times_{\Src f,\Dst f}Y$.
\item Both axioms seem to be consequences of
$f \sim X \times_{A_0, B_0} Y \Leftrightarrow f = X \times_{A_1, B_1} Y$
whenever
...
\begin{prop}
$\mathscr{S}(X\times Y)
=\mathscr{S}X\times^{\mathsf{pFCD}(\mathfrak{F}(\mho))}\mathscr{S}Y$
...
\begin{prop}
$\mathscr{S}\id_X=
\id^{\mathsf{pFCD}(\mathfrak{F}(\mho))}_{\mathscr{S}X}$
