...
\fxwarning{This subsection needs rewrite.}
...
\begin{thm}
$\setcond{ (\iota_{X,Y} f, \id^{\mathcal{C}(\Dst f)}_Y
\circ f \circ \id^{\mathcal{C}(\Src f)}_X) }{
f \in \mathcal{C} (A,B) }$ is a function and moreover is an
(order and semigroup) isomorphism, for objects $X\sqsubseteq\Src f$, $Y\sqsubseteq\Dst f$
for every partially ordered category~$\mathcal{C}$ with
restructed morphisms.
\end{thm}
