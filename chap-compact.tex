...
A funcoid $f$ is \emph{directly compact} iff
  \[ \forall \mathcal{F} \in \mathfrak{F}: (\supfun{f}
     \mathcal{F} \neq \bot \Rightarrow \supfun{\CoCompl f} \mathcal{F}
     \neq \bot). \]
\end{obvious}
...
\begin{prop}
A funcoid $f$ is directly compact iff $\dom\CoCompl f=\dom f$.
\fxerror{Wrong: $f=1^{\mathsf{FCD}}$ is a counter-example! Error in the book?}
\end{prop}

\begin{proof}
From the last ``obvious'' and proposition \bookref{x-dom-fcd} and its corollary~\bookref{dom-fcd-at}.
\fxnote{Specify which proposition exactly this follows from.}
\end{proof}
...
\fxwarning{It seems that the conditions~$\sqsubseteq$ and~$\sqsupseteq$ are interchanged for the classical topological space theory
and for subatomic product of funcoids. This makes both cases interesting (we should \emph{not} remove the theory with
subatomic product).}
...
Counter-example against $\langle f \times^{(A)} f \rangle^{\ast} 1^{\mathsf{RLD}} \sqsubseteq g\circ g^{-1}$:
...
\begin{prop}\label{vmv}
  ...
  \fxerror{Wrong: Counter-example above: $f=M=V=1^{\mathbf{Rel}}$.}
\end{prop}
...
\begin{prop}\label{ffm}
  ...
  \fxerror{The same counter-example.}
\end{prop}
...
\begin{thm}\label{cmp-main}
  Let $f$ be a $T_1$-separable compact \fxwarning{Isn't direct compactness enough? Yes, it is by symmetry.}
...
\begin{proof}
$\rsupfun{ f \times^{(A)} f } 1^{\mathsf{RLD}} \sqsupseteq g$ by proposition~\ref{ff-ge-g}.
