...
\begin{prop}
  ...
  \fxwarning{We can take $f$ either a funcoid or a reloid. If $f$ is a funcoid then
    replace $1^{\mathsf{RLD}}\rightarrow 1^{\mathsf{FCD}}$.}
  \begin{enumerate}
    \item\label{ff-id-s} $\langle f \times^{(C)} f \rangle^{\ast} 1^{\mathsf{RLD}}$;
      \fxwarning{We need to prove that $\langle f \times^{(C)} f \rangle^{\ast} 1^{\mathsf{FCD}}$
      is a filter (because it should be the set of neighborhoods of the diagonal, as per Bourbaki).}
      \fxnote{To settle this, we need to find a ``product'' of funcoids which agrees with product of topologies.}
    ...
    \item\label{ff-id-rcomp}  $\torldin (f \circ f^{- 1})$
  \end{enumerate}
\end{prop}
...
~
\begin{widedisorder}
\item[\ref{ff-id-s}$\Leftrightarrow$\ref{ff-id-at}] Theorem~\bookref{fcd-atoms} or
more generally theorem~\bookref{spfn-atoms} (it applies to funcoids due theorem~\bookref{fcd-diagram}).
\end{widedisorder}
??
\end{proof}
