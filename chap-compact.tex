...
\bigsqcup_{x\in\atoms 1^{\mathbf{Rel}}} \left( \dom x \times^{\mathsf{RLD}} \im x \right) \sqsupset
1^{\mathbf{Rel}}$.
\end{proof}

Statement~2 in page~172 of~\cite{bourb-top-1} does not survive modification:

\begin{example}\label{vmv}
  There is a funcoid~$f$ and $V \in \up f$ such that
  $V \circ M \circ V^{- 1} \notin \up \rsupfun{f
  \times^{(A)} f } M$.
\end{example}

\begin{proof}
Take $f=M=V=1^{\mathbf{Rel}}$ and use the example above.
\end{proof}
