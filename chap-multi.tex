...
\begin{thm}
\label{starcomp-is}Let $\Src f_{i}$ be a starrish join-semilattice
for every $i\in n$.
\begin{enumerate}
...
(taken into account that $\Src f_{i}$ is starrish) that is $\exists c\in\{0,1\}^{n}:(\mylamdba in{L_{c(i_{})}i})\in\GR\StarComp^{(a)}(a;f)$.
