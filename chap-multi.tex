...
\end{equation}
for every morphisms $f\in\Mor(A;B)$, $g\in\Mor(B;C)$, $h\in\Mor(A;C)$,
...
funcoid $\Mor(\Src f;\Src g)\rightarrow\Mor(\Dst f;\Dst g)$ defined
by the formulas (for every $a\in\Mor(\Src f;\Src g)$ and $b\in\Mor(\Dst f;\Dst g)$):
\[
...
and can be easily proved applying the formula (\ref{invertible})
twice.
\begin{prop}
...
\begin{proof}
For every funcoids $a\in\Mor(\Src f;\Src g)$ and $b\in\Mor(\Dst f;\Dst g)$
we have:
...
$i\in\arity f$ then $L\notin f$ if $f$ is a prestaroid.\end{prop}
\begin{proof}
...
\begin{prop}
\index{staroid!completary}A relation between posets whose form is
...
\begin{defn}
We will denote the set of staroids of a form $\mathfrak{A}$ correspondingly
...
finitely join-closed filtrators with both the base and the core being
...
It follows from the last proposition and the fact that it is finitely
join-closed.\end{proof}
\begin{prop}
Let each $(\mathfrak{A}_{i};\mathfrak{Z}_{i})$ for $i\in n$ (where
$n$ is an index set) is a finitely join-closed filtrator, such that
...
\begin{proof}
That $L\notin f$ if $L_{i}=\bot^{(\form f)_{i}}$ for some $i\in\arity f$
...
that where $(\core f)_{i}\subseteq(\base f)_{i}$ (thus having a filtrator
...
\begin{proof}
Let $i\in\arity f$ and $L\in\prod_{k\in L\setminus\{i,j\}}\mathfrak{Z}_{k}$.
...
starrish filtrators with separable finitely meet-closed core.\end{prop}
\begin{proof}
...
\setcond{L\in\prod\mathfrak{A}}{\forall K\in\up L:K\in\GR\prod^{\mathsf{Strd}}a} & \Leftrightarrow\\
...
L_{k}\in\supfun{\prod^{\mathsf{FCD}(\mathfrak{A})}A}_{k}L|_{(\dom\mathfrak{A})\setminus\{k\}} & \Leftrightarrow\\
...
\end{defn}
(Having fixed a precategory with star-morphisms) I will denote $\StarMor(P)$
the set of star-morphisms of the form $P$.
\begin{prop}
The sets $\StarMor(P)$ are disjoint (for different $P$).\end{prop}
\begin{proof}
...
set $\StarMor(X)$ is partially ordered for every $X$, such that:
\[
...
\begin{rem}
$\supfun{\chi f}=(f\circ-)$ is the $\Mor$-functor\footnote{Also called Hom-functor.}
$\Mor(f,-)$ and we can apply Yoneda lemma to it. (See any category
...
(\emph{cross-composition product} of $f$) from $\StarMor(\mylamdba i{\dom f}{\Src f_{i}})$
to $\StarMor(\mylamdba i{\dom f}{\Dst f_{i}})$ is defined by the
...
\begin{thm}
Let $\Src f_{i}$ be separable join-semilattice and $\Dst f_{i}$
...
\item [{\ref{fcomp-str}}] $\supfun{f_{i}}$ are monotone functions by
...
\exists x\in\Anch(\mathfrak{A})\setminus\{0\}:(x\sqsubseteq b\wedge x\sqsubseteq\StarComp^{(a)}(a;f)) & \Leftrightarrow\\
\exists x\in\Anch(\mathfrak{A})\setminus\{0\}:(x\sqsubseteq b\wedge\forall B\in\GR x:B\in\GR\StarComp^{(a)}(a;f)) & \Leftrightarrow\\
\exists x\in\Anch(\mathfrak{A})\setminus\{0\}:\\
...
\exists x\in\Anch(\mathfrak{A})\setminus\{0\}:\left(x\sqsubseteq b\wedge\forall B\in\GR x,A\in\GR a,i\in n:A_{i}\suprel{f_{i}}B_{i}\right) & \Leftrightarrow\\
...
and 
\begin{multline*}
\prod_{i\in\dom\mathfrak{A}}^{\mathsf{RLD}}\bigsqcap^{\mathscr{F}(\mathfrak{A}_{i})}\Pr_{i}S=\prod_{i\in\dom\mathfrak{A}}^{\mathsf{RLD}}\bigsqcap^{\mathscr{F}(\mathfrak{A}_{i})}\emptyset=\\
...
\end{proof}
\begin{conjecture}
$f\sqsubseteq\prod^{\mathsf{RLD}}a\Leftrightarrow\forall i\in\arity f:\Pr_{i}^{\mathsf{RLD}}f\sqsubseteq a_{i}$
for every multireloid $f$ and $a_{i}\in\mathscr{F}((\form f)_{i})$
for every $i\in\arity f$.
\end{conjecture}
...
\[
\left(\mathsf{FCD}(\StarMor(\Src\circ f);\StarMor(\Dst\circ f));\rsupfun{\uparrow^{\mathsf{FCD}}}\mathscr{P}\left(\prod(\Src\circ f)\times\prod(\Dst\circ f)\right)\right).
\]
\end{defn}
\begin{rem}
...
of filtrators where $i\in n$ for some index set $n$ (provided that
...
Our filtrators are with complete base by corollary \ref{filt-is-complete}.
\end{proof}
Because $\upuparrows f$ is a multifuncoid and $\mathord{\suprel f}=\mathord{\rsuprel{\upuparrows f}}$
...
which is down-aligned, with join-closed, finitely meet-closed and
...
\begin{proof}
That $L\notin\GR\ID_{\mathcal{A}[n]}^{\mathsf{Strd}}$ if $L_{k}=0$
...
\end{proof}
\begin{prop}
Let $(\mathfrak{A};\mathfrak{Z})$ be a finitely meet closed filtrator
