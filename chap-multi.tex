...
staroids on powersets.\end{example}
\begin{proof}
(based on an \noun{Andreas Blass}'s proof) Take $f$ the set of functions
$x:\mathbb{N}\rightarrow\subsets\mathbb{N}$ where $x_{0}$ is an arbitrary natural
...
% ??
% 
% Let take such a $\mathcal{L}_0\sqcup\mathcal{L}_1\in\up f$ that
% $(\lambda i\in\mathbb{N}:\mathcal{L}_{c(i)}(i))\notin f$.
% Each $\mathcal{L}_{c(i)}(i)$ is a filter.
% 
% To prove it, take $Y\in\up(\lambda i\in\mathbb{N}:\mathcal{L}_{c(i)}(i))$. Then each $Y_i$ is a subset of $\mathbb{N}$.
% We need to prove $Y\notin f$.
% 
% ??
...
\begin{defn}
$\prod^{\mathsf{RLD}}\mathcal{X}=\bigsqcap_{X\in\up\prod\mathcal{X}}^{\mathsf{RLD}(\mylamdba i{\dom\mathcal{X}}{\Base(\mathcal{X}_{i})})}\prod X$
...
\exists x\in\atoms\uparrow^{\mathsf{RLD}(\mylamdba i{\dom f}{\Src f_{i}})}X,y\in\atoms\uparrow^{\mathsf{RLD}(\mylamdba i{\dom f}{\Dst f_{i}})}Y:a\suprel{\prod^{(A)}f}b\Rightarrow\\
a\suprel{\prod^{(A)}f}b.
\end{multline*}
...
completary staroids of the form $\mylamdba i{\mathbb{N}}{\Base(a_{i})}$.\end{conjecture}
