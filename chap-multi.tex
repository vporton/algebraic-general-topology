...
\begin{proof}
Let $n=\mathbb{N}$. Let $\Base(a_{i})=\subsets\mathbb{R}$ and each $a_{i}$
be a principal filter corresponding to a two-element set.

Every $\prod_{i\in n}\left(\begin{cases}
A_{i} & \text{if }i\in m\\
\Base(a_{i}) & \text{if }i\in n\setminus m
\end{cases}\right)$ has at least $(\card\subsets\mathbb{R})^{n}=\card\subsets\mathbb{R}$ elements.

There are elements $\up\prod^{\mathsf{RLD}}a$ with cardinality $2^{n}=\card\mathbb{R}$.
They can't be elements of $\up\prod^{\mathsf{RLD}\ast}a$ because $\card\mathbb{R}<\card\subsets\mathbb{R}$.\end{proof}
\begin{cor}
There exists an indexed family $a$ of principal filters such that
...
\begin{lem}
\label{mf-f-base}$\setcond{\supfun f_{k}X}{X\in\up^{\prod_{i\in n\setminus\{k\}}\mathfrak{Z}_{i}}\mathcal{X}}$
