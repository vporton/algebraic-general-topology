...
\begin{proof}
Let $f=\setcond{\mathcal{A}\in\mathscr{F}(U)}{\uparrow\Cor\mathcal{A}\nasymp\Delta}$
for some infinite set $U$ where $\Delta$ is some non-principal
filter on $U$.
\begin{multline*}
A\sqcup B\in f\Leftrightarrow\uparrow^{U}\Cor(A\sqcup B)\nasymp\Delta\Leftrightarrow\uparrow^{U}\Cor A\sqcup\uparrow^{U}\Cor B\nasymp\Delta\Leftrightarrow\\
\uparrow^{U}\Cor A\sqcap\Delta\neq\bot^{\mathscr{F}(U)}\vee\uparrow^{U}\Cor B\sqcap\Delta\neq\bot^{\mathscr{F}(U)}\Leftrightarrow A\in f\vee B\in f.
\end{multline*}


Obviously $\bot^{\mathscr{F}(U)}\notin f$. So $f$ is a free star.
