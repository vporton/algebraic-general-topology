...
\begin{thm}
$\pi^{(J)}_i\prod^{(J)}f=f_i$.
\end{thm}

\begin{proof}
\begin{multline*}
\supfun{\pi^{(J)}_i\prod^{(J)}f}\mathcal{X} =\\
\supfun{\prod^{(J)}f\sqcap(\{i\}\times^{\mathsf{FCD}}\Dst \left(\prod^{(J)}f\right)_i}(\{i\}\times^{\mathsf{RLD}}\mathcal{X})=\\
...
$\supfun{\pi^{\rightarrow}_i}\mathcal{X} = \im\bigl(\mathcal{X}|_{\{i\}}\bigr)$.
\end{defn}
...
\begin{proof}
We need to prove $\pi^{\rightarrow}_i\circ\prod^{\rightarrow}f=f_i$.

Really, $\supfun{\pi^{\rightarrow}_i\circ\prod^{\rightarrow}f}\mathcal{X}=\supfun{\pi^{\rightarrow}_i}\supfun{\prod^{\rightarrow}f}\mathcal{X}=\supfun{\pi^{\rightarrow}_i}\coprod_{i\in\dom f}\supfun{f_i}\mathcal{X}=\supfun{\pi^{\rightarrow}_i}\prod_{i\in\dom f}(\{i\}\times^{\mathsf{RLD}}\supfun{f_i}\mathcal{X})=\supfun{f_i}\mathcal{X}$.
\end{proof}
