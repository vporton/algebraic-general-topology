...
\fxnote{Rewrite some theorems in terms of graphs of low spaces rather than low spaces themselves. This simplifies the formulas.}
...
\begin{lem}
For lower spaces~$f$, $g$ (on the same set)
\[
Q = \bigcup\rsupfun{\GR}S\cup Z\left(\bigcup\rsupfun{\GR}S\right)\cup Z\left(Z\left(\bigcup\rsupfun{\GR}S\right)\right)\cup\dots
\]
...
Then
\[
\mathcal{A},\mathcal{B} \in \underbrace{Z\dots Z}_{n\text{ times}}\left(\bigcup\rsupfun{\GR}S\right)
\]
for a natural~$n$. Thus
\[
\mathcal{A}\sqcup\mathcal{B} \in \underbrace{Z\dots Z}_{n+1\text{ times}}\left(\bigcup\rsupfun{\GR}S\right)
\]
...
\[
\GR\bigsqcup^{\mathbf{ASJ}}S = \bigcup\rsupfun{\GR}S\cup Z\left(\bigcup\rsupfun{\GR}S\right)\cup Z\left(Z\left(\bigcup\rsupfun{\GR}S\right)\right)\cup\dots
\]
\end{prop}

\begin{proof}
Denote $\mu$ the almost join space such that $\GR\mu = Q$ where $Q$ is the right part of the above formula. (It exists by the lemma.)
That $\mu$ is an upper bound of $S$ is obvious.

It remains to prove that $\mu$ is the least upper bound.

Suppose $\nu$~is an upper bound of $S$. Then $\GR\nu\supseteq\bigcup\rsupfun{\GR}S$. Thus, because $\GR\nu$ is an almost sub-join-semilattice,
$Z(\GR\nu)\subseteq\GR\nu$, likewise $Z(Z(\GR\nu))\subseteq\GR\nu$, etc. Consequently $Z(\bigcup\rsupfun{\GR}S)\subseteq\GR\nu$, $Z(Z(\bigcup\rsupfun{\GR}S))\subseteq\GR\nu$, etc.
So we have $Q\subseteq\GR\nu$ that is $\mu\sqsubseteq\nu$.
\end{proof}

\begin{prop}
~ \fxnote{Should be merged with the previous proposition.}
\begin{multline*}
\GR\bigsqcup^{\mathbf{ASJ}}S = \\
\setcond{F_0\sqcup\dots\sqcup F_{n-1}}{F_0,\dots,F_{n-1}\in\bigcup\rsupfun{\GR}S,F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-2}\nasymp F_{n-1} \text{ for } n\in\mathbb{N}}.
\end{multline*}
\end{prop}
...
\[F_0\sqcup\dots\sqcup F_{n-1}\in \underbrace{Z\dots Z}_{m\text{ times}}\left(\bigcup\rsupfun{\GR}\right)\]
for $F_0,\dots,F_{n-1}\in\bigcup\rsupfun{\GR}S$, $F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-2}\nasymp F_{n-1}$
and also $F_{n-1}\nasymp F_n$. Then $F_0\sqcup\dots\sqcup F_{n-1}\nasymp F_n$ and thus
$F_0\sqcup\dots\sqcup F_{n-1}\sqcup F_n\in \underbrace{Z\dots Z}_{m+1\text{ times}}\left(\bigcup\rsupfun{\GR}\right)$ that is
$F_0\sqcup\dots\sqcup F_{n-1}\sqcup F_n\in Q$. So $F\in Q$ for every~$F\in R$.

Now suppose $F\in Q$ that is for a natural~$m$
\[F \in \underbrace{Z\dots Z}_{m\text{ times}}\left(\bigcup\rsupfun{\GR}\right).\]
Let's prove by induction that $F=F_0\sqcup\dots\sqcup F_{n-1}$ for some $F_0,\dots,F_{n-1}\in\bigcup\rsupfun{\GR}S$
such that $F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-2}\nasymp F_{n-1}$.
If $m=0$ then $F\in\bigcup\rsupfun{\GR}S$ and our promise is obvious.
Let our statement holds for a natural~$m$. Prove that it holds for
\[F' \in \underbrace{Z\dots Z}_{m+1\text{ times}}\left(\bigcup\rsupfun{\GR}\right).\]
...
\begin{lem}
If $S$ is a set of lower spaces, then
\[
Q = \bigcup\rsupfun{\GR}S \cup Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right) \cup Z_{\infty}\left(Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right)\right) \cup \dots
\]
...
\begin{prop}
Join of a set~$S$ on the lattice of completely almost sub-join-semilattice is described by the formula:
\fxwarning{Define $\mathbf{CASJ}$.}
\[
\GR\bigsqcup^{\mathbf{CASJ}}S=\bigcup\rsupfun{\GR}S \cup Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right) \cup Z_{\infty}\left(Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right)\right) \cup \dots
\]
\end{prop}
...
\begin{prop}
\[
\GR\bigsqcup^{\mathbf{CASJ}}S=
\setcond{\bigsqcup T_0\sqcup\dots\sqcup\bigsqcup T_{n-1}}{
\begin{aligned}
&n\in\mathbb{N},T_0,\dots,T_{n-1}\in\bigcup\rsupfun{\GR}S, \\
