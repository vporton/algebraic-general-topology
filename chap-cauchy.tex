...
\begin{prop}
$\GR\bigsqcap S=\setcond{\bigsqcap\im P}{P\in\prod\rsupfun{\GR}S}$ for every set~$S$ of low spaces on some set.
\end{prop}

\begin{proof}
First prove that there is such low space~$\mu$ that $\GR\mu=\setcond{\bigsqcap\im P}{P\in\prod\rsupfun{\GR}S}$. In other words,
we need to prove that $\setcond{\bigsqcap\im P}{P\in\prod\rsupfun{\GR}S}$ is a nonempty lower set. That it is nonempty is obvious.
Let filter $\mathcal{G}\sqsubseteq\mathcal{F}$ and $\mathcal{F}\in\setcond{\bigsqcap\im P}{P\in\prod\rsupfun{\GR}S}$. Then
$\mathcal{F}=\bigsqcap P$ for a $P\in\prod\rsupfun{\GR}S$. Take $P'=(\mathcal{G}\sqcap)\circ P$. Then obviously
$\mathcal{G}=\bigsqcap P'$ and thus $\mathcal{G}\in\setcond{\bigsqcap\im P}{P\in\prod\rsupfun{\GR}S}$. So such~$\mu$ exists.

It remains to prove that $\mu$ is the greatest lower bound of~$S$.

?? $\mu$ is a lower bound of~$S$, because $\bigsqcap\im P\sqsubseteq x$ for every $x\in S$
that is $\GR\mu\subseteq\GR x$.
