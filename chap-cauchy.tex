...
\fxwarning{``Low filter spaces'' or ``Low spaces''?}
...
\begin{defn}
  ...
  filters on this set. \fxerror{I use both ``low filters space'' and ``low space''. Standardize.}
\end{defn}
...
\begin{rem}
In this version of the book we will consider almost sub-join-semilattices only on join-semilattices. I see no need to generalize it further.
\end{rem}
...
if $\bigsqcap T \neq \bot^{\mathscr{F}(X)}$ then $\bigsqcup T\in S$ for every $T\in\subsets S$. \fxnote{Can this be simplified?}
\end{defn}
...
\begin{defn}
  ...
  \begin{enumerate}
    ...
    \item $\mathscr{C}$ is a almost sub-join-semilattice.
  \end{enumerate}
\end{defn}

Without last axiom it is called \emph{precauchy space}.
...
\begin{defn}
  ...
  \begin{enumerate}
    \item $\mathscr{C}$ is nonempty.
    \item It is reflexive.
    \item $\mathscr{C}$ is a completely almost sub-join-semilattice.
  \end{enumerate}
\end{defn}
...
\fxnote{How reflexive low spaces correspond to reflexive reloids?}

\begin{defn}
A \emph{transitive} low space is such low space~$f$ that $(\mathsf{RLD})_{\Low}f\circ(\mathsf{RLD})_{\Low}f=(\mathsf{RLD})_{\Low}f$.
\fxwarning{Move this below definition of $(\mathsf{RLD})_{\Low}$.}
\end{defn}
...
\begin{prop}
  ...
  \fxwarning{Isn't it enough to be a completely almost sub-join-semilattice low space?}
  \fxnote{What about analogous binary join for Cauchy spaces?}
\end{prop}
...
\fxnote{Also do they preserve lattice operations for lattices of complete low filter spaces and of Cauchy spaces (and complete Cauchy spaces?).}
