...
\begin{prop}
For every set~$S$ of low spaces on some set \fxerror{Wrong: It is always the improper filter.}
\[
\GR\bigsqcap S=\rsupfun{\bigsqcap}\bigcup\rsupfun{\GR}S=\rsupfun{\bigsqcap}\GR\bigsqcup S=\bigsqcap\setcond{s}{P\in S,s\in\GR P}.
\]
\end{prop}
...
?? Isn't it weird that $\bigsqcap S$ can be restored knowing $\bigsqcup S$? Is it an error? For a counter-example take:
$f=[-1;1]\times[-1;1]$ and $g=[-2;2]\times[-2;2]$.
