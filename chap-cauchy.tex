...
\[
Q = \GR f\cup\GR g\cup Z(\GR f\cup\GR g)\cup Z(Z(\GR f\cup\GR g))\cup\dots
\]
is a graph of some almost sub-join space.
\end{lem}

\begin{proof}
That it is non empty and a lower set of filters is obvious. It remains to prove that it is an almost sub-join-semilattice.

Let $\mathcal{A},\mathcal{B}\in Q$ and $\mathcal{A}\nasymp\mathcal{B}$.
Then
\[
\mathcal{A},\mathcal{B} \in \underbrace{Z\dots Z}_{n\text{ times}}(\GR f\cup\GR g)
\]
for a natural~$n$. Thus
\[
\mathcal{A}\sqcup\mathcal{B} \in \underbrace{Z\dots Z}_{n+1\text{ times}}(\GR f\cup\GR g)
\]
...
\fxnote{The same formula describes join of Cauchy spaces.}
\[
\GR(f\sqcup^{\mathbf{ASJ}}g) = \GR f\cup\GR g\cup Z(\GR f\cup\GR g)\cup Z(Z(\GR f\cup\GR g))\cup\dots
\]
\end{prop}

\begin{proof}
Denote $\mu$ the almost join space such that $\GR\mu = Q$ where $Q$ is the right part of the above formula. (It exists by the lemma.)
That $\mu$ is an upper bound of $\{f,g\}$ is obvious.

It remains to prove that $\mu$ is the least upper bound.

Suppose $\nu$~is an upper bound of $\{f,g\}$. Then $\GR\nu\supseteq\GR f\cup\GR g$. Thus, because $\GR\nu$ is an almost sub-join-semilattice,
$Z(\GR\nu)\subseteq\GR\nu$, likewise $Z(Z(\GR\nu))\subseteq\GR\nu$, etc. Consequently $Z(\GR f\cup\GR g)\subseteq\GR\nu$, $Z(Z(\GR f\cup\GR g))\subseteq\GR\nu$, etc.
...
\[F_0\sqcup\dots\sqcup F_{n-1}\in \underbrace{Z\dots Z}_{m\text{ times}}(\GR f\cup\GR g)\]
for $F_0,\dots,F_{n-1}\in\GR f\cup\GR g$, $F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-2}\nasymp F_{n-1}$
and also $F_{n-1}\nasymp F_n$. Then $F_0\sqcup\dots\sqcup F_{n-1}\nasymp F_n$ and thus
$F_0\sqcup\dots\sqcup F_{n-1}\sqcup F_n\in \underbrace{Z\dots Z}_{m+1\text{ times}}(\GR f\cup\GR g)$ that is
$F_0\sqcup\dots\sqcup F_{n-1}\sqcup F_n\in Q$. So $F\in Q$ for every~$F\in R$.

Now suppose $F\in Q$ that is for a natural~$m$
\[F \in \underbrace{Z\dots Z}_{m\text{ times}}(\GR f\cup\GR g).\]
Let's prove by induction that $F=F_0\sqcup\dots\sqcup F_{n-1}$ for some $F_0,\dots,F_{n-1}\in\GR f\cup\GR g$
such that $F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-2}\nasymp F_{n-1}$.
If $m=0$ then $F\in\GR f\cup\GR g$ and our promise is obvious.
Let our statement holds for a natural~$m$. Prove that it holds for
\[F' \in \underbrace{Z\dots Z}_{m+1\text{ times}}(\GR f\cup\GR g).\]
...
\begin{prop}
Join on the lattice of Cauchy spaces \fxwarning{also on the lattice of almost sub-join spaces} is described by the formula
\fxnote{Does also the formula with ``chain'' of intersecting filters work?}
\fxnote{Remove above special cases of binary joins.}
\[
...
\[
\bigcup\rsupfun{\GR}S \cup Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right) \cup Z_{\infty}\left(Z_{\infty}\left(\bigcup\rsupfun{\GR}S\right)\right) \cup \dots
\]
\end{prop}
