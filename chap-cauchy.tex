...
$\mu$ is a lower bound of~$S$, because
for every $X\in S$ exists $P\in\prod_{X\in S}\GR X$ such that $P(X)=\GR X$ (taken into account that every $\GR X$ is nonempty)
and thus $\im P\ni\GR X$ and so $\bigsqcap\im P\sqsubseteq\GR X$, that is $\GR\mu\subseteq\GR X$.
