...
\begin{defn}
  I call \emph{low space} a set together with a nonempty lower set of proper
  filters on this set.
\end{defn}
...
~
\begin{widedisorder}
\item[\ref{low-rld-relf}] Let $f$ be reflexive. Then
  $1^{\mathsf{RLD}}\sqsubseteq f$; $\forall x\in\Ob f:\uparrow(\{x\}\times\{x\})\sqsubseteq f$;
  $\forall x\in\Ob f:\uparrow\{x\}\times^{\mathsf{RLD}}\uparrow\{x\}\sqsubseteq f$;
  $\forall x\in\Ob f:\uparrow\{x\}\in(\Low)f$; $(\Low)f$~is reflexive.
  
  Let now $(\Low)f$~be reflexive. ??
...
\item[\ref{rld-low-relf}] Let $f$ is reflexive. Then $\forall x\in\Ob f:\uparrow\{x\}\in f$;
  ...
  Let now $(\mathsf{RLD})_{\Low}f$~be reflexive. ??
\end{widedisorder}
\end{proof}
...
\fxwarning{Usage of the term ``Cauchy filter'' below for low spaces.}
