...
\[
\GR f\cup\GR g\cup\setcond{F\sqcup G}{F\in\GR f,G\in\GR g,F\nasymp G}.
\]
\fxerror{It seems that the above formula is wrong (not hard to invent a counter-example with a principal reloid), consider instead the recursive formula:}
\[
\GR f\cup\GR g\cup Z(\GR f\cup\GR g)\cup Z(Z(\GR f\cup\GR g))\cup\dots
\]
where $Z(f)=\setcond{F\sqcup G}{F\in f,G\in f,F\nasymp G}\cup\{\emptyset\}$.
\end{prop}
...
\[
\setcond{F_0\sqcup\dots\sqcup F_n}{F_0,\dots\,F_n\in\GR f\cup\GR g,F_0\nasymp F_1\land F_1\nasymp F_2\land\dots\land F_{n-1}\nasymp F_n \text{ for } n\in\mathbb{N}}.
\]
...
\begin{obvious}
$Z(f)\sqsupseteq f$ for every graph~$f$ of a low space.
\end{obvious}
