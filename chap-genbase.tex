...
\begin{proof}
Take $S = \up \id^{\mathsf{FCD}}_{\Omega}$. We know that $S$ is not a
filter base.

If $X \in \up \bigsqcap^{\mathsf{FCD}} S$ then $X \in \up
\id^{\mathsf{FCD}}_{\Omega}$ and then $X \in S$ and thus $S$ is a
base of a funcoid because $X \sqsubseteq X$.
\end{proof}

\fxnote{More generally than the above example, every $\up f$ for a funcoid~$f$ is a funcoid base.
Moreover $\up f$ for a funcoid can be restored knowing any its base.}
...
\begin{prop}
  ...
  \sqcap^{\mathsf{FCD}} Y)$ there is a $T \in S$ such that $T
  \sqsubseteq Z$.
  \fxnote{Try to prove by replacing a base of a funcoid~$f$ with~$\up f$.}
\end{prop}
