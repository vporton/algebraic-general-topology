...
\item[$\Leftarrow$] Let~$S$ be a base and is an upper set.
...
\begin{prop}
(For $\mathfrak{Z}$ having the greatest element) a base on the filtrator~$(\mathfrak{A};\mathfrak{Z})$ is the same
as base of a filter (on~$\mathfrak{Z}$). \fxwarning{for more general filtrators}
\end{prop}
