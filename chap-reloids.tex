...
\begin{defn}
Let $A$, $B$ be sets.
$\mathsf{RLD}[A;B]$ is the base of an arbitrary but fixed primary filtrator over $\subsets(A\times B)$.
(TODO: or over $\mathbf{Rel}(A;B)$ or over $\mathscr{T}(A\times B)$?)
\end{defn}

\begin{defn}
\index{reloid}I call a reloid from a set $A$ to a set $B$ a triple
$(A;B;F)$ where $F\in\mathsf{RLD}[A;B]$.
\end{defn}
