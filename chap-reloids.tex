...
\begin{defn} \fxwarning{Move below.}
$\up f^{-1} = \setcond{F^{-1}}{F\in\up f}$.
\fxwarning{Prove that is exists.}
\end{defn}
...
\begin{obvious}
$(\mathsf{RLD}(A;B);\mathbf{Rel}(A;B))$ is a powerset filtrator.
\end{obvious}
...
\begin{obvious}
The poset $\mathsf{RLD}(A;B)$ is isomorphic to the poset of filters on $A\times B$
and thus is a special case of $\mathsf{RLD}\sharp(A;B)$ (for every sets $A$, $B$).
\end{obvious}
