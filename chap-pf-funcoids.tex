...
the posets $\Src f$ and $\Dst f=\Src g$ have greatest elements and $\Src g$ and $\Dst g$ are strongly separable.
% and $\Dst g$ are strongly separable.
\item \label{pf-im-le-dom}If $\im f\sqsubseteq\dom g$ then $\dom(g\circ f)=\dom g$, provided that
the posets $\Dst g$ and $\Dst f=\Src g$ have greatest elements and $\Dst f$ and $\Src f$ are strongly separable.
% and $\Src f$ is separable.
\end{enumerate}
\end{prop}
\begin{proof}
~
\begin{widedisorder}
\item [{\ref{pf-im-ge-dom}}] $\im(g\circ f)=\supfun{g\circ f}\top^{\Src f}=\supfun g\supfun f\top^{\Src f}=\supfun g\im f\sqsupseteq
\text{(by strong separability of $\Dst g$)}\sqsupseteq\supfun g\dom g=\text{(theorem \ref{f-dom-f})}=\im g$.
By strong separability of~$\Dst g$ we have $\supfun g\im f\sqsubseteq\im g$.
...
and (\ref{pf-atoms-rel-cont}) follow from the theorem~\ref{spfn-atoms}
...
over boolean lattices. An $f\in\mathsf{pFCD}(\mathfrak{A};\mathfrak{B})$
...
Then taking into account the theorem~\ref{spfn-atoms} exists
...
~
\begin{multline*}
(a\times^{\mathsf{FCD}}b)\sqcap(f\sqcup g)\neq\bot^{\mathsf{pFCD}(\mathfrak{A};\mathfrak{B})}\Leftrightarrow a\suprel{f\sqcup g}b\Leftrightarrow a\suprel fb\vee a\suprel gb\Leftrightarrow\\
...
if $\mathfrak{A}$ has greatest element is defined and $\mathfrak{B}$ is a strongly separable meet-semilattice;
\item injective iff $f^{-1}\circ f\sqsubseteq\id_{\dom f}^{\mathsf{pFCD}(\mathfrak{A})}$,
if $\mathfrak{B}$ has greatest element is defined and $\mathfrak{A}$ is a strongly separable meet-semilattice. 
\end{itemize}
\end{prop}
\begin{proof}
