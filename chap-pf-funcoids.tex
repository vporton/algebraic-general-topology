...
Then for every $\mathcal{X}\in\mathfrak{A}$ \fxwarning{Define also $\mathcal{Y}$.}
\[
...
$\supfun{b\times^{\mathsf{FCD}}a}\mathcal{Y}\sqsubseteq\supfun{f^{-1}}\mathcal{Y}$ thus
...
that is \fxwarning{More detailed.} $\supfun{a\times^{\mathsf{FCD}}b}\mathcal{X}\sqsubseteq\supfun{f}\mathcal{X}$ and likewise
$\supfun{b\times^{\mathsf{FCD}}a}\mathcal{Y}\sqsubseteq\supfun{f^{-1}}\mathcal{Y}$, so
...
\begin{thm}
\label{pf-fcd-sep}\fxwarning{It seem that this theorem (and below theorems also) conditions can be weakened.}
Let $\mathfrak{A}$, $\mathfrak{B}$ be sets of
...
\end{prop}
\begin{proof}
$\mathfrak{B}$~is atomic by theorem~\ref{filt-atomic}.
\begin{align*}
