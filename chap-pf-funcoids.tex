...
\]
and thus by separability of $\mathfrak{B}$ we have $\supfun fx=\supfun gx$
that is $\supfun f=\supfun g$. Similarly we have $\supfun{f^{-1}}=\supfun{g^{-1}}$.
Thus $f=g$.
\end{widedisorder}
\end{proof}
\begin{prop}
...
$(\Dst f;\mathfrak{Z}_{1})$ are filtrators with separable core. Then
\[
...
be a finitely meet-closed filtrator with separable core which is a
...
Let's denote $W=\setcond{y\sqcap^{\Dst f}\rsupfun fX}{X\in\up^{(\Src f;\mathfrak{Z}_{0})}x}$.
We will prove that $W$ is a generalized filter base over $\mathfrak{Z}_{1}$.
To prove this enough to show that $V=\setcond{\rsupfun fX}{X\in\up^{(\Src f;\mathfrak{Z}_{0})}x}$
...
$A\sqcap^{\mathfrak{Z}_{0}}B\in\up^{(\Src f;\mathfrak{Z}_{0})}x$
(used the fact that it is a finitely meet-closed and theorem \ref{up-filt-crit})
...
\begin{equation}
\supfun f\mathcal{X}=\bigsqcap^{\mathfrak{B}}\rsupfun{\alpha}\up^{(\mathfrak{A};\mathfrak{Z}_{0})}\mathcal{X}\label{pf-alpha-filter}
\end{equation}
...
\begin{equation}
\mathcal{X}\suprel f\mathcal{Y}\Leftrightarrow\forall X\in\up^{(\mathfrak{A};\mathfrak{Z}_{0})}\mathcal{X},Y\in\up^{\mathfrak{Z}_{1}}\mathcal{Y}:X\mathrel\delta Y\label{pf-suprel-delta}
\end{equation}
...
by the formula $\corestar(\beta Y)=\setcond{Y\in\mathfrak{Z}_{1}}{X\mathrel\delta Y}$.
...
$Y\in\mathfrak{Z}_{1}$, then there exists exactly one funcoid $f$
such that $\langle f\rangle^{\ast}=\alpha$, $\langle f^{-1}\rangle^{\ast}=\beta$.\end{thm}
\begin{proof}
...
Thus by the theorem there exists a pointfree funcoid $f$ such that
$\langle f\rangle^{\ast}=\alpha$, $\langle f^{-1}\rangle^{\ast}=\beta$.
...
distributive lattice and $(\Dst f;\mathfrak{Z}_{1})$ is a primary
...
$(\Src f;\mathfrak{Z}_{0})$ is a finitely meet-closed filtrator by
...
\begin{prop}
$\supfun fx=\supfun f(x\sqcap\dom f)$ whenever $\dom f$ is defined,
for every $x\in\Src f$ for a pointfree funcoid $f$ whose source
is a meet-semilattice with least element and destination is a separable
poset with least element.\end{prop}
\begin{proof}
...
and source is a separable atomistic meet-semilattice.\end{cor}
\begin{proof}
...
$(\mathfrak{B};\mathfrak{Z}_{1})$ is finitely meet-closed by the
