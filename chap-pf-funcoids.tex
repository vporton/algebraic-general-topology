...
Now instead of considering funcoids, we will consider a more general case, \emph{pointfree funcoids}.
...
funcoids.
Pointfree funcoids are also used to define the semigroup of funcoids (see below).
...
defined by the formula $\im f=\bigsqcup_{x\in\mathfrak{A}}\supfun{f}x$.
\end{defn}

\begin{thm}
$f$ be a pointfree funcoid whose source is a set with greatest element.
$\im f=\supfun{f}\top$ for every $x\in\Src f$ whenever $\Dst f$
is a strongly separable poset with greatest element.
\end{thm}

\begin{proof}
$\supfun f\top$ is greater than every $\supfun fx$ (where $x\in\Src f$)
by proposition~\ref{pfcd-mono}.
\end{proof}
...
TODO: For unfixed funcoids this generalizes to the following proof:
$Y \nasymp \im f \Leftrightarrow Y \nasymp \bigsqcup_{x \in
\mathfrak{A}} \supfun{f} x \Leftrightarrow \exists x \in \mathfrak{A}: Y
\nasymp \supfun{f} x \Leftrightarrow \exists x \in \mathfrak{A}:
\supfun{f^{-1}} Y \nasymp x \Leftrightarrow \supfun{f^{-1}} Y
\nasymp \supfun{f^{-1}} Y \Leftrightarrow Y \nasymp \supfun{f}
\supfun{f^{-1}} Y \Rightarrow Y \nasymp \supfun{f} \dom f$;
$\supfun{f} \dom f \sqsupseteq \im f$. Alternative proof:
just limit to a suitable $\Hom$-set of funcoids.

\begin{prop}\label{pf-cap-dom}
$\supfun fx=\supfun f(x\sqcap\dom f)$
for every $x\in\Src f$ for a pointfree funcoid $f$ whose source
is a bounded separable meet-semilattice and destination is a bounded separable poset. \fxnote{Special case for unfixed funcoids with weaker conditions.}
\end{prop}
...
\begin{prop}\label{fx-im-fx}
$\supfun{f}x=\im(f|_x)$ if $f$ is an increasing pointfree funcoid.
\end{prop}

\begin{proof}
$\im(f|_x) = \bigsqcup_{a\in\mathfrak{A}}\supfun{f\circ\id_x}a =
\bigsqcup_{a\in\mathfrak{A}}\supfun{f}(x\sqcap a)$ what because $\supfun{f}$ is monotone equal to $\supfun{f}x$.
\end{proof}

\begin{prop}\label{x-dom-f}
$x\nasymp\dom f\Leftrightarrow(\supfun fx\text{ is not least})$ for
every pointfree funcoid $f$ and $x\in\Src f$ if $\Dst f$ has greatest
element $\top$.
\fxnote{Need to prove for unfixed funcoids: just limit to a suitable $\Hom$-set of funcoids.}
\end{prop}
...
and source is an atomistic poset.
\fxnote{Need to prove for unfixed funcoids.}
\end{prop}
...
Prove the further for unfixed funcoid, too:

\begin{prop}\label{a-domf}
...
\end{cases}=\rsupfun{A\times B}X$. Likewise $\supfun{(A\times^{\mathsf{FCD}}B)^{-1}}Y=\rsupfun{(A\times B)^{-1}}Y$.
\end{proof}
