...
\item a functor $\Upsilon = (f\mapsto\supfun{f})$ from $\Src\Upsilon$
  ...
  \fxwarning{We can extend to a wider set of posets.}
...
\begin{defn}
I call a system of \emph{distributive} sides a system of sides with additional axiom: \fxwarning{Weaker axiom: $\supfun{a}$ is monotone.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item $\supfun{a}(a\sqcup b)=\supfun{a}a\sqcup\supfun{a}b$.
  \item $\supfun{a} \bot = \bot$
  \item $\supfun{a \sqcap b} X \sqsubseteq \supfun{a} X \sqcap \supfun{b} X$
\end{enumerate}
\end{prop}
