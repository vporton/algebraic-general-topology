...
Category of \emph{sides} $\Upsilon$ is an ordered category
whose objects are (small) bounded lattices and whose morphisms are maps
between lattices such that every Hom-set is a bounded lattice and (for all
relevant variables):
\begin{enumerate}
  \item $(a \sqcup b) X = a X \sqcup b X$
  
  \item $(a \sqcap b) X \sqsubseteq a X \sqcap b X$ (equivalent to monotonicity of morphisms?)
  
  \item $(\lambda x \in \mathfrak{A}: x \sqcap c) \in \Upsilon (\mathfrak{A};
  \mathfrak{A})$ for every $c \in \mathfrak{A}$
  
  \item $a \bot = \bot$
  
  \item $\top X = \top$ unless $X = \bot$
\end{enumerate}
I call morphisms of such categories \emph{sides}.

\fxnote{Redefine sides as $f\rightarrow\supfun{f}$ function? Then we can probably consider reloids as sides?}

\begin{obvious}
$1^{\mathfrak{A}} \in \Upsilon(\mathfrak{A};\mathfrak{A})$.
\end{obvious}
...
\begin{prop}
  The element $1^{\Upsilon (\mathfrak{A}; \mathfrak{A})}$ is not
  complemented if $\mathfrak{A}$ is a non-atomic boolean lattice,
  for every category~$\Upsilon$ of sides.
\end{prop}

\begin{proof}
  Let $T = 1^{\Upsilon (\mathfrak{A}; \mathfrak{A})}$.
  
  Let's suppose $T \sqcup V = \top$ for $V \in \mathsf{pFCD} (\mathfrak{A};
  \mathfrak{A})$ and prove $T \sqcap V \neq \bot$.
  
  Then $( T \sqcup V ) a = \top$ for all $a \neq \bot$ and thus $V
  a \sqcup a = \top$.
  
  Consequently $V a \sqsupseteq \neg a$ for all $a \neq \bot$.
  
  if $a$ isn't an atom, then there exists $b$ with $0 \sqsubset b \sqsubset a$
  and hence $V (a) \sqsupseteq V (b) \sqsupseteq \neg b \sqsupset \neg a$;
  thus $V a \sqsupset \neg a$.
  ...
  For $a \nsqsubseteq c$ we have $V a \sqcap a \sqsupset \bot$
  for all $a \sqsubseteq \neg c$ thus $(T \sqcap V) a \sqsupseteq
  V a \sqcap a \sqsupset \bot$. Thus $((T \sqcap V) \circ
  \id_{\neg c} ) a \sqsupset \bot$
