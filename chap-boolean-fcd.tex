...
The following negative result is modeled after theorem~3.8 in~\cite{tprod-dist-lat}.
It seems interesting to find a common generalization of my below theorem and the theorem after which it was modeled.

\begin{prop}
  The element $1^{\mathsf{pFCD} (\mathfrak{A}; \mathfrak{A})}$ is not
  complemented if $\mathfrak{A}$ is a non-atomic boolean lattice.
\end{prop}

\begin{proof}
  Let $T = 1^{\mathsf{pFCD} (\mathfrak{A}; \mathfrak{A})}$.
  
  Let's suppose $T \sqcup V = \top$ for $V \in \mathsf{pFCD} (\mathfrak{A};
  \mathfrak{A})$ and prove $T \sqcap V \neq \bot$.
  
  Then $\langle T \sqcup V \rangle a = \top$ for all $a \neq \bot$ and thus $V
  a \sqcup a = \top$.
  ...
  for all $a \sqsubseteq \neg c$ thus $\langle T \sqcap V \rangle a \sqsupset
  V a \sqcap a \sqsupset \bot$. Thus $\langle (T \sqcap V) \circ
  \id_{\neg c} \rangle a \sqsupset \bot$
...
\begin{cor}
  $\mathsf{pFCD} (\mathfrak{A}; \mathfrak{A})$ is not boolean if $\mathfrak{A}$
