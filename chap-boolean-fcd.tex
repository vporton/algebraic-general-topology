...
\begin{thm}
\label{fcd-diagram}Let $A$ and~$B$ be fixed sets. The diagram consisting of solid arrows of the diagram at the figure~\ref{rels-dia} is
a commutative diagram (in category $\mathbf{Set}$), every arrow (including the dashed arrows) in
...
\end{claim}
\begin{claimproof}
Let $r_0\in\subsets(A\times B)$ and $f\in\mathscr{T}A\otimes\mathscr{T}B$ \fxwarning{Define this notation.} 
...
That $\Phi^{-1}$ is the inverse of~$\Phi$ is obvious.
...
\begin{claim}
Every cycle going through solid arrows in the diagram~\ref{rels-dia} is identity.
\end{claim}
\begin{claimproof}
...
\begin{claim}
Subdiagram of diagram~\ref{rels-dia} where we limit only to solid arrows is commutative.
\end{claim}
\begin{claimproof}
From the previous claim.
\end{claimproof}

\fxwarning{It remains to prove that the arrows preserve order, composition, and reversal.}

\end{proof}

Informally speaking, dashed arrows replace a relation~$r$ with its complement relations~$\lnot r$. Formally:

\begin{prop}
$\Psi_2^{-1} \Phi^{-1} \Psi_4 \Psi_2 r = \neg r$ for every relation~$r$ between~$A$ and~$B$.
\end{prop}
...
\nasymp \supfun{f} \{ x \}$; $\{ y \} \nasymp \neg \rsuprel{r} \{ x \}$; $\{ y \} \mathrel{\not{\sqsubseteq}} \rsuprel{r} \{ x \}$; $y \notin \rsupfun{r} \{ x \}$.
