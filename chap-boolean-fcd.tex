...
preserve composition. Note that for a Galois connection~$f$ I denote $f_0$ the lower adjoint and $f_1$ the upper adjoint.
For brevity I will denote a pointfree funcoid between~$\subsets A$ and~$\subsets B$ as $(\supfun{f};\supfun{f^{-1}})$ instead of $(\subsets A;\subsets B;\supfun{f};\supfun{f^{-1}})$.

\begin{figure}[ht]
% \begin{sidewaysfigure}
\caption{\label{rels-dia}}
% \begin{sideways}
\begin{tikzcd}[row sep=4cm, column sep=2cm, ampersand replacement=\&]
\& \begin{tabular}{c}binary relations\\ between $A$ and $B$\end{tabular}
\arrow[rd, shift left, "r\mapsto\left(X\mapsto\setcond{y\in B}{\forall x\in A:x\mathrel{r}y}; Y\mapsto\setcond{x\in A}{\forall y\in B:x\mathrel{r}y}\right)"]
...
\arrow[rr, shift left, "\begin{aligned}\scriptscriptstyle f\mapsto\left(X\mapsto\lnot\supfun{f}X;Y\mapsto\bigsqcap_{y\in\mathscr{T} Y\setminus\{\bot\}}\supfun{f^{-1}}y\right)=\\
  \scriptscriptstyle\left(X\mapsto\lnot\supfun{f}X;Y\mapsto\bigsqcap_{y\in Y}\supfun{f^{-1}}\{y\}\right)\end{aligned}"]
\arrow[rd, shift left, "{\begin{aligned}&\scriptscriptstyle f\mapsto\left(\supfun{f};Y\mapsto\bigsqcup_{y\in\mathscr{T}Y\setminus\{\top\}}\supfun{f^{-1}}\lnot y\right)=\\
&\scriptscriptstyle \left(\supfun{f};Y\mapsto\bigsqcup_{y\in Y}\supfun{f^{-1}}\lnot\{y\}\right)\end{aligned}}"]
\& \& \begin{tabular}{c}antitone Galois\\ connections\\ between\\ $\subsets A$ and $\subsets B$\end{tabular}
\arrow[lu, shift left, "\begin{aligned}\scriptscriptstyle f\mapsto\setcond{(x;y)}{y\in f_0\{x\}}=\\\scriptscriptstyle\setcond{(x;y)}{x\in f_1\{y\}}\end{aligned}"]
\arrow[ll, shift left, "\begin{aligned}\scriptscriptstyle f\mapsto\left(X\mapsto\lnot f_0 X;Y\mapsto\bigsqcup_{y\in\subsets Y}f_1 y\right)=\\
  \scriptscriptstyle\left(X\mapsto\lnot f_0 X;Y\mapsto\bigsqcup_{y\in Y}f_1\{y\}\right)\end{aligned}"]
\arrow[ld, leftrightarrow, "f\mapsto(\mathord{\lnot}\circ f_0;f_1\circ\mathord{\lnot})"]
\\
\& \begin{tabular}{c}Galois connections\\ between $\subsets A$ and $\subsets B$\end{tabular}
\arrow[lu, shift left, "\begin{aligned}\scriptscriptstyle f\mapsto\left(f_0;Y\mapsto\bigsqcap_{y\in\subsets Y}f_1\lnot y\right)=\\
  \scriptscriptstyle\left(f_0;Y\mapsto\bigsqcap_{y\in Y}f_1\lnot\{y\}\right)\end{aligned}"]
\end{tikzcd}
% \end{sideways}
\end{figure}
% \end{sidewaysfigure}
