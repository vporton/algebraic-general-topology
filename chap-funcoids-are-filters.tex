...
\bigsqcap^{\mathsf{FCD}}\setcond{X\times Y\cup\overline{X}\times B}{X\in\subsets A,Y\in\subsets B,\left\langle \bigsqcap^{\mathsf{FCD}}\up\bigsqcap^{\mathsf{RLD}}\up\mathcal{A}\right\rangle X\sqsubseteq Y}\left\{ \right\}  & =\\
...
({*}{*}) by theorem \ref{fcd-as-meet} in \cite{volume-1}.

Similarly 
\[
\up\mathcal{B}=\bigsqcap^{\mathsf{FCD}}\setcond{X\times Y\cup\overline{X}\times B}{X\in\subsets A,Y\in\subsets B,\left\langle \bigsqcap^{\mathsf{FCD}}\up\mathcal{B}\right\rangle X\sqsubseteq Y}.
\]
...
if $\bot\neq\mathcal{X}\sqsubseteq A$ and $\supfun{A\times^{\mathsf{FCD}}Y\sqcup\overline{A}\times^{\mathsf{FCD}}\top}\mathcal{X}=1\in\up\langle F\rangle\mathcal{X}$
if $\mathcal{X}\nsqsubseteq A$.
...
\section{Relationships between $\protect\tofcd$ and $(\mathsf{RLD})_{\Gamma}$}
\begin{defn}
...
\item $\im f=\bigsqcap^{\mathscr{F}(A)}\langle\im\rangle^{\ast}\up^{\Gamma(A;B)}f$.
\end{enumerate}
\end{prop}
\begin{proof}
