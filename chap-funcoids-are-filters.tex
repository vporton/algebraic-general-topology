...
\begin{thm}\label{fcd-comp-ent}
$\forall H\in\up(g\circ f)\exists F\in\up f,G\in\up g: H\sqsupseteq G\circ F$ for every composable funcoids $f$ and $g$.
\end{thm}

\begin{proof}
Obviously $\setcond{G\circ F}{F\in\up f,G\in\up g}$ is a filter base.
Thus our thesis holds by theorem~\ref{fcd-comp-up} and properties of generalized filter bases.
\end{proof}
