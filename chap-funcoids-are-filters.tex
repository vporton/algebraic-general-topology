...
\fxerror{Noted Sep 10 2016:
An erroneous example said that $(\mathsf{RLD})_{\Gamma}\ne\torldin$ but
from theorem~\ref{fcd-diagram} it follows that they are equal. Rewrite knowing that
$(\mathsf{RLD})_{\Gamma}=\torldin$.
}
...
\begin{thm}
$\tofcd(\mathsf{RLD})_{\Gamma}f=f$ for every funcoid $f$.\end{thm}
\begin{proof}
...
$\tofcd$ and $\torldin\neq(\mathsf{RLD})_{\Gamma}$ \fxerror{(See example
?? below.)}
