...
\fxnote{I yet haven't succeed to prove it for a larger class of spaces such as all topological vector spaces.}
...
for a direction~$d$ and an angle~$\phi$. \fxwarning{I use $R$ in two different senses. Change the notation.}
...
\fxnote{This probably can be generalized for $C^n$ and $C^\infty$ curves. Consider also fractional~$n$
(see \url{https://en.wikipedia.org/wiki/H\%C3\%B6lder_condition} for a classical definition (Sobolev spaces or Hoelder spaces), also
\url{https://en.wikipedia.org/wiki/Sobolev_inequality}).
What about curves limited by an exponent?}
...
Define $R^n(d;\phi) = \setcond{v\in R}{\widehat{vd}<\phi |v|^n, v\ne 0}$ for $d\ne 0$ and $R^n(0;\phi) = \{0\}$.

\fxnote{Alternatively define
$R^n(d;\phi) = \setcond{v\in R}{\forall h<n:\widehat{vd}<\phi |v|^h, v\ne 0}$ for $d\ne 0$ and $R^n(0;\phi) = \{0\}$.
Also express the second definition of~$R$ through the first one.}

\begin{defn}
$W^n(d) = \bigsqcap^{\mathsf{RLD}}\setcond{R^n(d;\phi)}{\phi\in\mathbb{R},\phi>0} \sqcap \bigsqcap^{\mathsf{RLD}}_{r>0}B_r(0)$.
\end{defn}
...
~
\begin{enumerate}
\item A path~$f$ is $C^n$ smooth iff
$f\in\continuous(\iota_D|\mathbb{R}|_{>};Q_+^m)\cap\continuous(\iota_D|\mathbb{R}|_{<};Q_-^m)$
for all $m<n$.
\item $f\in\continuous(\iota_D|\mathbb{R}|_{>};Q_+^m)\cap\continuous(\iota_D|\mathbb{R}|_{<};Q_-^m)$ for a path~$f$ iff
it is $C^n$ smooth for all $m<n$ (this requires $C^n$ for fractional~$n$).
\end{enumerate}
\end{conjecture}
