...
\begin{defn}
A \emph{simple}\footnote{I am not an expert in $\mathrm{d}$-spaces, probably experts call it differently.} $\mathrm{d}$-space
is such a $\mathrm{d}$-space that it is impossible to extract non-constant subpath which is also a $\mathrm{d}$-path from a $\mathrm{d}$-path
(if we do not count a path as a subpath of itself).
\end{defn}

\begin{conjecture}
Every simple $\mathrm{d}$-space $(X;d)$ is generated by the pair $(X;\nu)$ of funcoids where
$\nu = \bigsqcup_{a\in d}(a\circ |\mathbb{R}|_{\geq}\circ a^{-1})$.
\end{conjecture}

\begin{conjecture}
A simple $\mathrm{d}$-path~$a$ is determined by the funcoids (where $x$ spans $[0;1]$)
