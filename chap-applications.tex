...
Let $d$ be a vector field in a normed vector space.
\fxnote{I yet haven't succeed to prove it for a larger class of spaces such as all topological vector spaces.}

\fxwarning{Below the interval $[0;1]$ and the set~$\mathbb{R}$ are confused with each other.}
...
\begin{defn}
A differentiable function $f$ is an \emph{integral curve} of the vector field~$d$ when $f'(t)=d(f(t))$.
\end{defn}

\begin{defn}
A \emph{reparameterization} of a $C^1$ path is a bijective $C^1$~real function $\phi$ such that
...
\begin{proof}
Equivalently transform $f\in\continuous(\iota_{[0;1]}|\mathbb{R}|;Q_+)$; $f\circ \iota_{[0;1]}|\mathbb{R}|\sqsubseteq Q_+\circ f$;
$\rsupfun{f\circ \iota_{[0;1]}|\mathbb{R}|}@\{t\}\sqsubseteq \rsupfun{Q_+\circ f}@\{t\}$;
$\rsupfun{f}\Delta_{>}(t)\sqcap[0;1])\sqsubseteq \rsupfun{Q_+}f(t)$;
