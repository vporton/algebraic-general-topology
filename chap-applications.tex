...
See \url{http://math.stackexchange.com/questions/1882625/a-modification-of-integral-curves/1882696} for
description of reparametrized integral curves.
...
\begin{thm}
A $D^1$ function~$y$ is some reparameterization of a $D^1$ integral curve~$x$ of vector field~$d$ iff
$f'(t)\upuparrows d(f(t))$ that is vectors~$f'(t)$ and $d(f(t))$ have the same direction (for every $t$).
\end{thm}
...
Let now $f'(t)\upuparrows d(f(t))$ that is that is there is a strictly positive function $c(t)$ such that
$x'(t) = c(t) F(x(t))$.

??
\end{proof}

\subsection{The slope field}
