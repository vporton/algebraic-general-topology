...
\begin{thm}
$f$ is an integral curve iff
\end{thm}

\fxnote{The condition that left and right derivatives are the same is equal to continuity in $\mathbb{R}^{n+1}$.}

\fxnote{Possible alternative definitions of~$Q$}
\[ I = ([0;1]\times[0;1]) \sqcap \bigsqcap^{\mathsf{FCD}}_{\epsilon>0} \setcond{(x;y)}{y>x, y<x+\epsilon} =
...
a function in $\continuous(I_+;Q_+)\cap\continuous(I_-;Q_-)$.
Here $I_+$ is the complete funcoid defined by the formula
\[ \rsupfun{I_+}\{t\} = (\Delta_{+} + t)\sqcap[0;1]. \]
\end{thm}
...
$f'(t)=d(f(t))$ is equivalent to $f$ being a continuous function on~$\mathbb{R}^{n+1}$ with direction specified by
$F(v(t), 1)$.
