...
\fxnote{Prove that product of one multiplicant is equal to this multiplicant!}
\end{defn}
...
\begin{thm}
If morphisms $\pi_i X$ are metainjective and ??, then
$\prod^{(L)}$ is a categorical product.
\end{thm}

\begin{proof}
First prove $\pi_k(\Dst\circ F)\circ\prod^{(L)}F=F_k$. Really
$\pi_k\prod^{(L)}F=
\pi_k(\Dst\circ F)\circ\bigsqcap_{i \in \dom F} \bigl((\pi_i(\Dst\circ F))^{\dagger} \circ F_i \circ \pi_i(\Src\circ F)\bigr)
=\text{(metainjectivity)}=
\bigsqcap_{i \in \dom F} \bigl(\pi_k(\Dst\circ F)\circ(\pi_i(\Dst\circ F))^{\dagger} \circ F_i \circ \pi_i(\Src\circ F)\bigr)
=\text{(monovaluedness and surjectivity)}=??=
\bigsqcap_{i \in \dom F} \bigl(F_i \circ \pi_i(\Src\circ F)\bigr)
$
...
\bigotimes f = f_i$; $\bigotimes_{i \in n} (\pi_i X \circ f) = f$. \fxwarning{How are $\pi_i X$ and $f$ related?}
\end{rem}
...
If the category~$\mathcal{C}$ is dagger, we will also make $\cont(\mathcal{C})$ a dagger category: $(f,a,b)^{\dagger}=(f^{\dagger},b^{\dagger},a^{\dagger})$. It is really a dagger category:
...
$\mathbf{Set}$. \fxwarning{How are they defined in $\mathbf{Set}$?}
...
The below definitions are wrong, because $\prod^{(L)}$ is (apparently) not a categorical product.

I will define~$\mathbf{Fcd}$ as the category of endofuncoids and continuous functions between them.

It is obvious that this is really a category.

I define product (??prove it's a product) in this category as (??consider the infinite case):

$A\times B = A\times^{(L)} B$

$f\times g$ is the usual product of two functions.

First need to prove $\pi_0: A\times B\to A$ (and symmetrically): ??
