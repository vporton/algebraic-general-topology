...
Let $\coprod^{(Q)}X$ be an object for each inddexed family~$X$ of objects.

Let $\iota$ be a partial function mapping elements $X\in\dom\iota$ (which consist of small indexed families of objects of~$\mathcal{C}$) to indexed families $X_i\to\coprod^{(Q)}X$ of principal morphisms (called \emph{injections}) for every $i\in\dom X$.

We will denote particular morphisms as $\iota_i^X$.

If (but we won't assume this below) $\iota_i \xleftrightarrow{\dual} \pi_i$ that is $\iota_i =
...
by anlaogy with $\operatorname{proj}$ counterparts replacing~$\pi$ by~$\iota^{\dagger}$.
...
\begin{thm}
$\coprod^{\text{inj}}_{\sqcap}$ together with $\iota$ is a categorical coproduct in the category $\cont (\mathcal{C})$.
\end{thm}

\begin{proof}
Dual of theorem~\ref{cont-pr-pr}.
\end{proof}
...
\item For each objects $A$, $B$ is given an object $\operatorname{MOR} ( A , B)$ (//exponentiation//) and a morphism $\varepsilon_{A, B} : \operatorname{MOR} ( A , B) \times A \rightarrow B$.
