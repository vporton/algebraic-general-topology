...
Then evaluation is $\varepsilon_{B, C} = \mathop{\sim} 1_{\operatorname{MOR}(B,C)}$.

So $\varepsilon_{B, C} ( p , q) = ( \mathop{\sim} 1_{\operatorname{MOR}(B,C)}) ( p , q) = 1_{\operatorname{MOR}(B,C)} ( p) ( q) = p ( q)$.

''Proposition'' Evaluation is a morphism of $\mathbf{Dig}$.

''Proof'' Because $\varepsilon_{B, C} ( p , q) = \mathop{\sim} 1_{\operatorname{MOR}(B,C)}$.

It remains to prove:
\begin{itemize}
\item $\varepsilon_{B, C} \circ ( \sim f \times 1_{A}) = f$ for $f : A \rightarrow B \times C$;
\item $\sim ( \varepsilon_{B, C} \circ ( g \times 1_{A})) = g$ for $g : A \rightarrow \operatorname{MOR} ( B , C)$.
\end{itemize}
