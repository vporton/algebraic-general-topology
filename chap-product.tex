...
\begin{rem}
  ...
  However there are also some important examples where it is partial. \fxwarning{How do we define it for partial?}
\end{rem}
...
\fxwarning{Need to specify the source and destination because
$\dom F$ may be empty.}
\end{defn}
...
\subsection{Coproduct for endomorphisms}
...
\begin{defn}
$\pi^{\cont(\mathcal{C})}_i = \left(F_i,\bigodot^{\operatorname{inj}}_{\sqcup}F,\iota_i\right)$.
\end{defn}

\begin{obvious}
$\iota_i$ are continuous that is $\pi^{\cont(\mathcal{C})}_i$ are morphisms.
\end{obvious}
