...
\begin{cor}
  $\pi_i (\lambda j\in n:\Ob F_j) \in \mathrm{C} \left( \prod^{(L)} F , F_i \right)$ for every $i \in
  \dom F$.
\end{cor}
...
\begin{rem}
We can consider also a function~$\bigotimes$ of an indexed family of morphisms, values of which are morphisms. For some categories $\pi_i \circ
...
\begin{defn}
  $\pi^{\cont (\mathcal{C})}_i F = \left( \pi_i(\lambda j\in n:\Ob F_j), \prod^{(L)} F , F_i \right)$.
\end{defn}
...
\begin{enumerate}
\item $\pi^{\operatorname{cont}(\mathcal{C})}_i\prod^{(L)}_{j\in??}(f_j,a_j,b_j) = (f_i,a_i,b_i)$;
\item $\prod^{(L)}_{i\in\dom F}(\pi^{\operatorname{cont}(\mathcal{C})}_i\circ F) = F$.
\end{enumerate}
??
\end{proof}
...
Abbreviate $\iota_i = \iota_i(\lambda j \in n : \Ob F_j)$.
