...
\begin{thm}
In the category $\cont (\mathcal{C})$:
\begin{enumerate}
\item $\prod^{\text{proj}}_{\sqcap}$ together with $\pi$ is a categorical product.
\item $\coprod^{\text{proj}}_{\sqcap}$ together with $\pi$ is a categorical coproduct.
\end{enumerate}
\end{thm}

\begin{proof}
I'll prove only the first, as the other is dual.
\fxwarning{For coproduct use injections not projections.}
