...
Then $\prod^{(L)} F = \bigsqcap_{i \in \dom F} ((\pi^{\lambda j \in n :
...
\begin{itemize}
\item Exponential object $\operatorname{HOM(A;B)}$ is:
\item\begin{itemize}
\item Object $\operatorname{Ob}\operatorname{HOM(A;B)} = (\operatorname{Ob} B)^{\operatorname{Ob} B}$;
\item Graph is $\operatorname{GR} \operatorname{HOM} ( A ; B) = \uparrow^{\mathsf{FCD}} \setcond{ ( f ; g) }{ f, g \in \operatorname{Mor}_{\mathbf{Set}} (\operatorname{Ob} A;\operatorname{Ob} B) \wedge \uparrow^{\mathsf{FCD}}g \circ A \circ \uparrow^{\mathsf{FCD}}f^{- 1} \sqsubseteq B }$.
\end{itemize}
