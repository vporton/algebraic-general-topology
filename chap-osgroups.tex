...
More generally, we can define a \emph{generalized space} as a homomorphism of ordered groups. Most of the below however does use poset elements, so it could not be described in terms of generalized spaces.
...
However, we can define compactness without specifying~$\mathfrak{Z}$ as we can take~$\mathfrak{Z}$ to be the center of the poset~$\mathfrak{A}$. \fxwarning{Do I define center for an arbitrary poset?}
